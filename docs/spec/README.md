...
| [SPEC-072](SPEC-072-TOWER-CALLABLE-TYPE-AND-CLOSURE-SYNTAX.md) | Tower Callable Type and Closure Syntax | Draft | Defines tower-aligned callable arrows `->`, `-*>`, `=>`, and `=*>`; makes `(A, B) -> C` the preferred pure callable type spelling; switches pure closure shorthand to `|args| -> body`; and reserves Act/Proc/Workflow callable and closure arrows until their application semantics are implemented. |
