...
| [SPEC-056](SPEC-056-FIRST-CLASS-WORKFLOW-CARRIER.md) | First-Class Workflow Carrier | Draft | `Workflow<A>` as a contract-indexed `Proc<A>` carrier with Monad-shaped `workflow` operations, coverage/evidence relations, `do:Workflow`, and `[...]: Workflow` comprehension integration |
