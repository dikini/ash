...
- `[...]: Workflow` comprehensions reuse the same typed-do normalization path.
...
Reference-only examples document intended algebra/lift spellings that are already covered at lower parser/typechecker layers, but are not yet end-to-end source-file examples because `parse_file` still lowers some typed workflow expressions before typed elaboration.
...
- `05-workflow-comprehension.ash`: `[...]: Workflow` comprehension target spelling.
