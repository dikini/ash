...
checklist:
  - [ ] Create acceptance-delta audit artifact.
  - [ ] Map every TASK-974 deferred gap to exactly one owning follow-on task.
  - [ ] Name exact implementation files, test files, test names, and expected RED failure modes.
  - [ ] Confirm Phase 127 history is not reopened.
  - [ ] Replace all downstream fail-closed placeholder verification blocks.
  - [ ] Update PLAN-123 if audit discovers different file targets or sequencing.
```
