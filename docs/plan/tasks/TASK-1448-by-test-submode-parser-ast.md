# TASK-1448: Parse/preserve authored/property/small-world `by test` submodes

## Status: ✅ Complete

## Description

Parse/preserve authored/property/small-world `by test` submodes. This task is part of Phase 145 / PLAN-145 and implements a small, reviewable slice of [SPEC-081](../../spec/SPEC-081-LAW-TEST-EVIDENCE-SUBSTRATE.md). The task must preserve the core phase constraint: final user-facing law/test/proof behavior is runnable with an Ash-under-test executable (`$ASH_UNDER_TEST test ...`) and does not require the law/test/proof author or executor to use Cargo or Rust tooling. Do not assume the globally installed `ash` is current while Ash tooling is under development.
...
1. Keep `by test "name"` accepted and classify it as authored test evidence.
2. Add parser/AST support for explicit `by test authored`, `by test property`, and `by test small_world` if accepted by SPEC-081 MVP decisions.
3. Reject malformed submode blocks with diagnostics instead of degrading to `Expr` proof bodies.
4. Add parser roundtrip/extraction tests for each submode.
...
Ground the syntax in `surface.rs` and `parse_module.rs`; do not invent unrelated property-test syntax.
