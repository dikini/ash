...
| [SPEC-073](SPEC-073-ASHGROVE-INSTALL-UPDATE-CLEANUP-GIT-DEPLOYMENT.md) | Ashgrove Install, Update, Cleanup, and Git Deployment | Draft | Defines `ashgrove <command>` as the user-local Ash toolchain/deployment manager: source and binary tarball installs, immutable XDG-local toolchain bundles containing `ash`, `ashgrove`, stdlib, and selected standard tooling, update/default/remove/cleanup policy, and lower-case `ash.toml` git URL + tag/rev dependencies resolved into exact commits in `ash.lock` before registry package management. TASK-974 closeout evidence is complete, and PLAN-123/TASK-975..986 now track the remaining release, trust, registry-ready metadata, cleanup reachability, runtime-support, and closeout rows before this spec can move beyond Draft. |
