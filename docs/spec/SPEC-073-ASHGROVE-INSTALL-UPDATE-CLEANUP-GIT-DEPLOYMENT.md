...
Current implementation note: the TASK-968 and TASK-977 follow-up slices prove the `ash-engine` selected-stdlib override seam with a temporary installed-style stdlib root and source-archive release metadata. Source installs now build real local source checkouts from an isolated cache copy with an external Cargo target dir, stage immutable toolchains through the staged publish path, record source URL/revision/build profile/target triple plus dirty/unidentified override and reproducibility state, reject dirty or unidentified source roots unless the matching explicit override is present, fail closed when git-like roots cannot report `HEAD` or dirty status, reject source archives without typed `release-source.toml` origin-commit metadata unless `--allow-unidentified-source` is explicit, record `source_archive_digest` and `source_origin_commit` for identified source archives, reject same-id metadata conflicts, keep identical reinstalls deterministic, copy source stdlib package metadata into `lib/ash/std/ash.toml`, preserve existing bundled runtime stdlib support modules under `lib/ash/std/src/runtime/`, and refresh launcher shims that pass the selected toolchain stdlib root to `ash`. SPEC-073 remains Draft because Phase 128 still owns concrete runtime-support payload metadata through TASK-978 and later deferred rows.
