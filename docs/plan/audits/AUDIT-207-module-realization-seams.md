...
(active TASK-2075, `partial / tested / below_spec` after its visibility and private-carrier
prerequisites). The internal
...
| Collection and binding | `canonical_provisional_module_scopes.rs` collects only structural children and ordinary functions for bounded TASK-2068/TASK-2070 routes. TASK-2075 supplies tested visibility carriers plus closed private/read-only collection carriers, typed keys, one paired map, module sidecars, domain/source fences, and private removed-`Capability` validation. The graph collector still publishes no internal snapshot or name-only view. | Preserve the compatibility facade. TASK-2075 must populate and atomically publish its paired views; TASK-2072 consumes only the name view; TASK-2073 consumes the internal snapshot plus staged bindings. | TASK-2071 (Complete contract), TASK-2075 (In progress, partial/tested/below-spec), TASK-2072/TASK-2073 (planned) |
