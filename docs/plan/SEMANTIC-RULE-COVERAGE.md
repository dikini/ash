...
- **Binding metadata revalidation:** Imported declaration visibility is checked against the acquired checked target; same-identity visibility drift rejects atomically before interface publication with `BindingVisibilityMismatch`. A forged imported lookup namespace or declaration kind is rejected at the same boundary with `BindingShapeMismatch`, and a forged local alias is rejected against the authoritative import-map key with `BindingLocalNameMismatch`, before any interface can publish.
- **Updated focused target:** The qualified namespace, implementation-call, qualified implementation-operation, where-bound, transitive-row, callable-whole-row, role-row, policy-row, binding-metadata, forged-binding-shape, and forged-local-name RED/GREEN slices leave the focused TASK-2073 integration target at 93/93, plus two dedicated finalizer unit witnesses.
...
- **Activation and implementation evidence:** `crates/ash-typeck/tests/task_2073_checked_module_finalization.rs` passes 93/93, and `canonical_checked_module_finalizer::tests::forged_imported_binding_namespace_rejects_atomically` plus `canonical_checked_module_finalizer::tests::forged_imported_binding_local_name_rejects_atomically` pass as dedicated unit witnesses. Existing positive and negative evidence remains as previously recorded; the qualified namespace, qualified implementation-operation, where-bound, transitive-row, callable-whole-row, role/policy-row, binding-metadata, forged-shape, and forged-local-name evidence above extends the public implementation closure slice. These are tests, not proof or downstream execution/client parity.
...
- **Next obligation:** Extend the bounded finalizer to remaining declaration facts while keeping named policy bindings deliberately transient and minimal (local alias, defining identity, policy namespace, provenance, and public schema only), and complete forged/incomplete/cyclic dependency and visibility/export-closure rejection, including imported namespace dependency visibility, while preserving downstream ownership boundaries.
