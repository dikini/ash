...
- [x] Legacy `plays role(...)`, `requires:` and `ensures:` header events lower through the same shared `WorkflowForm` lowering/projection substrate used by first-class workflow forms.
- [x] Source-ordered legacy `plays role(...)`, `requires:` and `ensures:` header events are preserved in translation.
