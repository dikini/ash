...
**M-SELF-SIMPLE-ALIAS active seam:** TASK-2070 owns a `partial / none / below_spec` Type-only
...
will select only direct same-module ordinary functions when `is_visible_from` permits the
...
valid-sibling/failing-module boundaries will be atomic. `CanonicalBoundModuleBinding` and the
generic binder remain unchanged. The one implementation node and eight witnesses are deferred;
...
| Graph identity | `crates/ash-parser/src/canonical_module_graph.rs::CanonicalModuleGraphResolver` publishes an all-or-nothing parser graph keyed by `ModuleKey`, with real acquired file/inline `ModuleUnit` values, root metadata, AST-only edges, complete lifecycle reporting, and anchored structural diagnostics. Its `ModuleResolver`/`ModuleGraph` compatibility path is isolated behind the explicitly deprecated legacy surface; `semantic_summary::ModuleIdentity` is unchanged. | Pass the produced, non-authorizing parser graph/unit handoff to the completed TASK-2068 foundation, then TASK-2070/2071/2072/2073. | TASK-2067 (Complete), TASK-2068 (Complete foundation), successors TASK-2070–2073 |
| Imports | `crates/ash-parser/src/import_resolver.rs` and Engine loader have distinct import paths; TASK-2061 adds only an in-memory explicit/group/glob resolver over checked wrappers | TASK-2068 preserves its bounded tested planners/binders. TASK-2070 owns only the direct self alias; TASK-2072 owns complete parsed grammar, edge/cycle/precedence/duplicate semantics and staged `pub use`; TASK-2073 alone finalizes exports before TASK-2069 fences Engine readers. | TASK-2070 (In progress), TASK-2072/TASK-2073 (planned), TASK-2069 (planned) |
| Binding | `crates/ash-typeck/src/canonical_module_binder.rs` provisionally collects parsed functions and binds simple crate-root aliases, retaining identity/origin/span/visibility; `canonical_function_interface.rs` adds graph-preflighted primitive-function sibling checking and fresh identity/private-public projection; `canonical_primitive_provider_client.rs` checks only a root plus plan-selected direct providers | Preserve every delivered TASK-2068 bounded fragment unchanged. TASK-2071 completes collection, TASK-2072 completes parsed binding/precedence/cycles/staged re-exports, and TASK-2073 completes bodies/private-public/export closure before TASK-2069 consumes a complete checked handoff. | TASK-2068 (Complete foundation), TASK-2071/2072/2073 (planned) |
...
TASK-2071 through TASK-2073 own the remaining parsed import/interface authority. These tasks must update this table whenever a
...
layer. TASK-2068 is complete for its partial/tested bounded foundation. TASK-2071 owns remaining
provisional namespaces, TASK-2072 parsed imports/binding, and TASK-2073 checked bodies/final
...
|---|---|---|---|
| MOD-REAL-001 | A module declaration creates one stable child identity and structural edge from parsed AST | partial / tested / below_spec | TASK-2067 completes canonical AST-only `ModuleKey` edges, anchored missing/root+nested-duplicate/malformed-inline/cycle failures, complete lifecycle reporting, parsed-source invalid-key rejection, canonical-key rewrite resistance, and an isolated legacy-route fence; TASK-2068's completed foundation and TASK-2071 are the next non-authorizing consumers |
| MOD-REAL-002 | File and inline sources produce equivalent module units after acquisition | partial / tested / below_spec | TASK-2067 retains actual file/inline units in its canonical graph and proves complete ordered payload parity and payload-only mutation at that boundary. TASK-2073 owns checked Type-layer final-interface parity, TASK-2069 lowering transport, and TASK-2064 terminal parity |
| MOD-REAL-003 | Checked public interfaces are export-closed and preserve defining identities | partial / tested / below_spec | TASK-2060 Core carrier and TASK-2066 bounded wrapper complete; TASK-2068 provides only its tested leaf foundation. TASK-2071 completes provisional collection and TASK-2073 owns full body/callable facts, typed linkage, aliases/re-exports, final interfaces, and export closure |
| MOD-REAL-004 | Imports and visibility resolve only through finalized checked interfaces | partial / tested / below_spec | TASK-2061 completed only a wrapper-store explicit/group/glob resolver; TASK-2068's bounded parsed leaves remain evidence. TASK-2070 owns the self-alias leaf, TASK-2072 owns complete grammar/visibility/re-exports/cycles/precedence/binding, and TASK-2073 finalizes exports |
