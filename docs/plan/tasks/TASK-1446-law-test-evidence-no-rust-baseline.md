# TASK-1446: Audit and freeze no-Rust CLI baseline for law/test evidence

## Status: ✅ Complete

## Description

Audit and freeze no-Rust CLI baseline for law/test evidence. This task is part of Phase 145 / PLAN-145 and implements a small, reviewable slice of [SPEC-081](../../spec/SPEC-081-LAW-TEST-EVIDENCE-SUBSTRATE.md). The task must preserve the core phase constraint: final user-facing law/test/proof behavior is runnable with an Ash-under-test executable (`$ASH_UNDER_TEST test ...`) and does not require the law/test/proof author or executor to use Cargo or Rust tooling. Do not assume the globally installed `ash` is current while Ash tooling is under development.
...
1. Identify the currently installed `ash`, determine whether it is current enough, and define the Phase 145 `$ASH_UNDER_TEST` candidate policy when it is not.
2. Record whether `--include-synthesized laws` and `--only-synthesized laws` are available without Cargo on both the installed entrypoint and the selected `$ASH_UNDER_TEST` candidate.
3. Create minimal fixtures that demonstrate the current authored/property/small-world/by-test behavior and its gaps.
4. Patch downstream task verification commands or closeout notes if `$ASH_UNDER_TEST` must point somewhere other than the ordinary `ash` entrypoint.
...
This task produces the baseline artifact every later task must improve. Do not implement semantic fixes here; document drift and create fail-closed fixtures.
