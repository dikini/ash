...
- Added Core Ash text parsing for full expression fixtures, supported effect operation forms, handler clauses with explicit resume parameters, continuation references, and trap reasons. (TASK-1623)
- Added canonical Core Ash text serialization and file output APIs with parser/serializer round-trip coverage for representative `.core` expressions. (TASK-1624)
- Added the basic Core Ash validator boundary with raw/validated program wrappers, row duplicate detection, recursive type/value/expression checks, and malformed effect operation rejection. (TASK-1625)
- Added conservative Core Ash affine handler resume validation, accepting zero or one direct resume jump and rejecting duplicate jumps, ordinary argument passing, lambda capture, and record/tuple storage. (TASK-1626)
- Added basic Core Ash to CPS lowering for representable values, lets, primitive operations, conditionals, tail calls, jumps, row-field synthesis, and non-tail calls through `LetCall` to CPS `LetCont`. (TASK-1627)
- Added Core Ash effect/discharge lowering for capability and failure raises, handlers with affine resume binding, contract `RecordDischarge`, traps, and contract-violation row exclusion. (TASK-1628)
- Added end-to-end Core Ash fixture tests that parse `.core`, validate Core, lower to CPS, serialize/reparse CPS terms, compare stable `.cps.golden` files, and reject an invalid duplicate-row fixture. (TASK-1629)
- Added Core Ash reference documentation for the `.core` fixture/debug text format and implemented validated Core-to-CPS lowering boundary, with docs consistency tests tied to committed fixtures. (TASK-1630)
- [Phase 161](docs/plan/PLAN-161-CORE-ASH-IR-FOUNDATION.md): Implemented the Core Ash IR foundation with dedicated Core AST carriers, strict `.core` fixture/debug text format, parser/serializer round-trips, Core validation, Core-to-CPS lowering, end-to-end CPS goldens, reference docs, and closeout review. Surface-to-Core lowering, ad-hoc polymorphism, arbitrary user-defined algebraic effects, `MultiShotPure`, Core `Match`, and full type checking remain out of scope. (TASK-1620 through TASK-1631)
