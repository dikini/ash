...
async fn stdlib_logging_wrappers_emit_structured_records_and_redacted_evidence() {
    ...
    let source = r#"
        fn main() -> Int {
            do {
                debug_event <- logging::debug("debug secret=alpha");
                info_event <- logging::info("info secret=bravo");
                warn_event <- logging::warn("warn secret=charlie");
                error_event <- logging::error("error secret=delta");
