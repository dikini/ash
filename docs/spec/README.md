...
| [SPEC-056](SPEC-056-FIRST-CLASS-WORKFLOW-CARRIER.md) | First-Class Workflow Carrier | Draft | `Workflow<A>` as a contract-indexed `Proc<A>` carrier with a blocking `WorkflowForm`/projection/obligation semantic gate, source-ordered legacy `WorkflowHeaderEvent`s, non-denotable contract argument classes, Monad-shaped `workflow` operations, coverage/evidence relations, legacy-compatible `requires:`/`ensures:` contract syntax, `workflow::requires`/`workflow::ensures` intrinsic contract injection, deprecated legacy workflow declaration translation to the same semantics, delayed lower Proc/Act coverage obligations, `do:Workflow`, and `[...]: Workflow` comprehension integration |
