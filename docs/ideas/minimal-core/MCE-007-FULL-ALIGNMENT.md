...
| Spawn / completion observation contracts | Surface, IR, big-step, and small-step inputs are accepted, including the upstream completion-observation contract. Runtime evidence confirms spawn-time control authority and pause/resume/kill/check-health boundaries through `ControlLinkRegistry`; TASK-406 adds one explicit retained completion-observation carrier in `ash-interp` with write-once/sealed semantics; TASK-407 adds a runtime-owned child workflow registry plus real spawned-child execution path keyed by `workflow_type`, allowing the runtime to automatically seal retained completion from the child lifecycle without regressing supervisor control semantics; TASK-408 preserves direct child terminal success/error results in `RetainedCompletionRecord.result`; TASK-409 adds `RetainedCompletionRecord.effects` with `effects.terminal_upper_bound` plus conservative `effects.reached_upper_bound`; TASK-410 adds `RetainedCompletionRecord.obligations` with terminal-visible local pending obligations plus terminal-visible active-role pending/discharged state; TASK-411 adds `RetainedCompletionRecord.provenance` with runtime-owned child identity plus spawn lineage; and TASK-412 adds `RuntimeState::wait_for_retained_completion(&ControlLink)` so callers can await the first sealed retained record directly. | `true residual drift`: retained completion observation is now materially stronger, but the current carrier remains intentionally minimal rather than full `CompletionPayload` contents; its effect summary is conservative rather than exact; and its obligations slice is only the terminal-visible subset the current runtime can honestly snapshot. This row therefore stays open independently of the broader cumulative-carrier work. |
