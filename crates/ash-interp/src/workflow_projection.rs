...
//! crates: shapes that are already sound (`unit`, materialized `bind`/`then`,
//! transparent `scope`) execute, and all remaining first-class Workflow
...
/// Full `from_proc` and `from_act` scheduling is still outside this slice;
/// those shapes return a named Phase 108 unsupported diagnostic.
pub fn execute_workflow_proc_projection(
    ...
    match projection {
        ...
        WorkflowProcProjection::Bind { source, next, .. } => {
