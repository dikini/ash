...
- Remediated independent review blockers by retaining transitive promoted field-domain dependencies in selected type-function summaries, hiding selected proposition promoted data-kind dependency metadata to avoid source-visible alias leakage, making associated-family promoted-carrier conversion fail closed without production panics, tracking/enforcing promoted data-kind and promoted constructor dependencies in public type-function export closure, preserving hidden promoted data-kind payloads through imported-summary and `pub use` merge paths, and promoting merged selected summaries carrying promoted data kinds to V6.
- Verification evidence: focused TASK-894/TASK-895/TASK-896 tests plus `cargo test -p ash-engine --lib task896_`, `cargo test -p ash-typeck --test task_896_promoted_constructor_integration`, and `cargo test -p ash-typeck task896_public_type_function --lib --tests` pass after review remediation. Broad closeout gates are `cargo fmt --check`, `git diff --check`, `cargo check --workspace`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo test --workspace`, `cargo doc --workspace --no-deps 2>&1 | tee /tmp/ash-plan-114-doc.log`, and `! grep -i '^warning:' /tmp/ash-plan-114-doc.log` after the final code/doc change.