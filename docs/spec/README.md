...
| [SPEC-074](SPEC-074-ASHGROVE-SOURCE-PAYLOAD-LOCAL-STATE-IGNORE.md) | Ashgrove Source Payload and Local-State Ignore Policy | Draft | Amends SPEC-073 source-root install semantics by separating reproducible source payload from local checkout state: gitignored and known local-state paths such as `.agents/`, nested `target/`, worktrees, and caches must not affect source-root payload digest or isolated build copy, while nonignored source payload changes remain fail-closed and source-archive attestation behavior remains separate. |
