...
Completed focused verification for TASK-852:

```bash
cargo test -p ash-typeck --test task_827_normalizer_diagnostics task_827_concrete_normal_form_required_reports_neutral_not_inversion -- --exact --nocapture
cargo test -p ash-typeck --test task_852_type_computation_summary_diagnostics -- --nocapture
cargo test -p ash-engine --test task_852_engine_summary_diagnostics -- --nocapture
cargo fmt --check
git diff --check
...
The TASK-827 regression check preserves the older open-neutral concrete-normal-form-required diagnostic while TASK-852 maps only unsupported neutral computation applications to unavailable-private-reduction. Independent re-review after remediation reported no remaining blockers.
