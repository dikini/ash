# TASK-1490: Runner Strategy Resolution

## Status: ✅ Complete

## Description

Teach the test runner to resolve explicit strategy overrides first and `Arbitrary<T>` evidence second.
...
- ✅ TASK-1488: prerequisite task in this phase
- ✅ TASK-1489: prerequisite task in this phase
...
1. Parse explicit strategy override metadata or parser-level override syntax chosen by TASK-1485.
2. Resolve parameter `x: T` by override -> generator/shrinker bridge -> `Arbitrary<T>` -> built-in fallback -> defer/error.
3. Record resolution source in JSON repro artifacts.
4. Add negative tests for unknown strategy, wrong type, duplicate strategy, and missing Arbitrary evidence.
...
strictness: clean
commands:
  - cargo test -p ash-cli strategy_resolution
  - cargo fmt --check
  - git diff --check
checklist:
