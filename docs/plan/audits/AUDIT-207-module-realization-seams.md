...
(active TASK-2075, `partial / tested / below_spec` after Task 5's atomic paired collector,
namespace/collision, parent/member/constructor, typed notation/diagnostic, and full impl-head
coherence evidence). The internal
...
| Collection and binding | `canonical_provisional_module_scopes.rs` remains the bounded TASK-2068/TASK-2070 compatibility route. TASK-2075 now atomically publishes paired internal/name-only views across the expanded graph, with exhaustive namespace disposition, parent-aware collisions and member/constructor identities, internal-only impl facts, typed notation keys and diagnostics, full alpha-normalized impl-head coherence within the current/lexical interface scope, and late-sibling failure rejection; imported interface identities fail closed pending TASK-2072 binding. | Preserve the compatibility facade. TASK-2075 still owes complete raw-fact/drift/file-inline/generated/property/authority evidence; TASK-2072 consumes only the non-authorizing name view; TASK-2073 consumes the internal snapshot plus staged bindings. | TASK-2071 (Complete contract), TASK-2075 (In progress, partial/tested/below-spec), TASK-2072/TASK-2073 (planned) |
