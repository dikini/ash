...
| TCIR/AMIR entry artifacts | checked-function artifact and computation provenance carriers | workflow artifact carriers | target Core/TCIR/AMIR computation artifacts with effect rows | Refactor to target primitive; implementation verification remains open | TASK-1972 |
...
| `WorkflowDef` residual carrier | current contract-only workflow-form structures | workflow declaration AST | remove residual parser/lowering carriers; target function contracts are current path | Plan required | TASK-1970-A |
...
| TASK-1971 in-progress | Removed workflow-form parser/lowering carriers and stale tooling/test metadata: parser `WorkflowHeaderEvent`, `WorkflowDef.header_events`, `WorkflowDef.contract`, and header adaptation; surface `Definition::Proxy`; LSP proxy completion/hover/navigation/indexing; the unused typechecker `DefinitionKind::Proxy`; and the obsolete `#[workflow]` macro test. Program entries use target function metadata. | RED/GREEN Phase 201 removal gates cover parser/lowering carriers; focused LSP navigation/index tests, `ash-lsp-core` all-target checks, `ash-macros` tests/checks, and `ash-typeck` checks verify the surviving target forms. | TASK-1971 remains open for the broader strict remove-only audit, but the active parser surface no longer exposes proxy or workflow declaration forms and current tooling does not advertise them. |
| TASK-1972 in-progress | Runtime artifact construction now consumes a checked function artifact (identity, Core row, result type, source anchor, and lowered Core body) instead of synthesizing a null-bodied application entry from a text-derived row summary. TCIR/AMIR provenance records the checked function metadata and body fingerprint. | Focused real-source CLI, engine identity-mismatch/body-provenance, and cache-identity RED/GREEN regressions cover the path. | Cross-crate verification and closeout review remain open; do not treat this row as complete until those checks confirm the cache/provenance path. |
