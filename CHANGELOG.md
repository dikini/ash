...
### Added
- [TASK-967](docs/plan/tasks/TASK-967-toolchain-metadata-and-xdg-layout.md): Added a typed ashgrove launcher-dispatch helper that resolves bundled tool paths by explicit override, project `ash.toml` pin, then user default, with fail-closed missing-toolchain diagnostics under temporary XDG roots. TASK-967 remains partial because real `$HOME/.local/bin` launcher shim creation/execution is still deferred.
