...
### Fixed
- ADT constructor typing, exhaustiveness, and runtime pattern tests now follow one constructor-shaped contract (TASK-174). `ash-typeck` now resolves variant patterns from canonical enum metadata instead of synthetic `__variant` record tags, exhaustiveness witnesses preserve required constructor field shape, and new `ash-typeck` / `ash-interp` contract tests lock in constructor-shaped ADT behavior end to end.
