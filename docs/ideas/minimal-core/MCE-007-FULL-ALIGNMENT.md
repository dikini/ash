...
|---|---|---|---|---|---|
| Sequencing / binding / branching | ✅ | ✅ | Backbone fixed; per-form correspondence still to package | partial / follow-up required — ordinary residual execution is directly realized by recursive execution over residual `Workflow` ASTs with direct `Γ` carriage and distributed `A = (C, P)` holders, and TASK-433 now adds one runtime-owned `ExecutionRecord` substrate with direct terminal projection plus cumulative obligation/provenance/trace/effect carriage on the main execution path. Follow-up remains because that packaging is still first-slice and not yet the fully uniform runtime story across `Par`, retained completion, and every runtime-facing observation surface. | partial / follow-up required |
...
| Parallel composition | ✅ | ✅ | Backbone fixed; interleaving + helper aggregation settled | partial / follow-up required — TASK-435 now upgrades `Workflow::Par` from coarse successful value collation to branch-local execution-record aggregation: each branch runs with its own recorder/provenance context and the enclosing `Par` execution record is rebuilt from branch-local trace/effect/obligation/provenance snapshots rather than one shared recorder. Follow-up still remains for a fully explicit branch-step/interleaving machine and any stronger helper-backed concurrent aggregation closure beyond this landed execution-record slice. | partial / follow-up required |
| Capability / policy / obligation workflows | ✅ | ✅ | Backbone fixed; detailed correspondence still to package | partial / follow-up required — ambient capability/policy context is distributed across runtime holders and policy checks are evidenced at decision/action/receive boundaries; obligation state is genuinely carried in `Context` / `RoleContext`; and TASK-433 now provides one authoritative execution-record substrate that packages terminal-facing obligation/provenance/trace/effect state on the main execution path. Follow-up remains because those cumulative carriers are still not yet uniformly packaged across all runtime-owned observation and aggregation paths. | partial / follow-up required |
| Spawn / completion observation contracts | ✅ | ✅ | Helper/runtime contract fixed as input | partial / follow-up required — spawn-time control authority and later pause/resume/kill/check-health boundaries are directly realized through `ControlLinkRegistry`. TASK-406 adds a sealed/write-once retained terminal observation carrier, TASK-407 adds a runtime-owned child workflow registry plus real spawned-child execution path keyed by `workflow_type` that automatically seals retained completion from the child lifecycle while preserving live supervisor control authority after spawn, TASK-408 preserves one honest `CompletionPayload.result`-like field (`RetainedCompletionRecord.result: Option<Box<ExecResult<Value>>>` with `terminal_result()`), TASK-409 adds a conservative `CompletionPayload.effects`-like summary slice (`RetainedCompletionRecord.effects: Option<ConservativeRetainedEffectSummary>` with `conservative_effect_summary()`, `effects.terminal_upper_bound`, and conservative `effects.reached_upper_bound`), TASK-410 adds one honest terminal-visible `CompletionPayload.obligations`-like slice (`RetainedCompletionRecord.obligations: Option<ConservativeRetainedObligationsSummary>` with `conservative_obligations_summary()`), TASK-411 adds one runtime-owned retained provenance slice (`RetainedCompletionRecord.provenance: Option<ConservativeRetainedProvenanceSummary>` with `conservative_provenance_summary()`), and TASK-412 adds one dedicated retained-completion wait surface via `RuntimeState::wait_for_retained_completion(&ControlLink) -> Result<RetainedCompletionRecord, ControlLinkError>`. Full `CompletionPayload` parity remains absent. | partial / follow-up required |
...
- Observable-preservation verdict: the current interpreter partially realizes the accepted MCE-005 backbone for observable purposes. Successful return class is direct, non-success is visible but multiplexed, and TASK-433 now introduces one authoritative execution-record substrate carrying terminal-facing `Ω` / `π` / `T` / `ε̂` slices on the main execution path. Uniform runtime-wide packaging and aggregation of those carriers still remains partial/follow-up work.
- Spawn/completion authority and retained-completion limits: control authority lifecycle is directly realized and stable after spawn registration. TASK-406 adds one equally explicit retained runtime carrier for minimal completion-style observation and seals it write-once, TASK-407 adds a runtime-owned child workflow registry plus narrow spawned-child entry contract (`init` bound into child context) so `Workflow::Spawn` can launch a real child execution path keyed by `workflow_type`, TASK-408 preserves the child's direct terminal `Result<Value, ExecError>` in `RetainedCompletionRecord.result` when the retained observation comes from child completion, TASK-409 preserves a conservative retained effect summary in `RetainedCompletionRecord.effects` (`effects.terminal_upper_bound` plus conservative `effects.reached_upper_bound`), and TASK-410 preserves one honest retained obligations slice in `RetainedCompletionRecord.obligations` (terminal-visible local pending obligations plus terminal-visible active-role pending/discharged state). That real child lifecycle therefore now drives automatic retained completion sealing without eagerly terminating the child inline, but the retained carrier is still limited to coarse outcome/state, direct terminal result, conservative effect summary, conservative terminal-visible obligations summary, and source-kind metadata rather than full `CompletionPayload` contents.
...
|---|---|---|---|---|
| Sequencing / binding / branching | `mixed: accepted partiality + true residual drift` | Direct residual execution itself is accepted as aligned, and TASK-433 now narrows the gap by introducing one authoritative execution-record substrate on the main execution path, but fully uniform cumulative `Ω` / `π` / `T` / `ε̂` packaging is still not realized across all runtime-facing surfaces. | future runtime/interpreter follow-on for carrier packaging; TASK-400 for closeout wording | keep row open and explicitly note the split between accepted local execution alignment, landed first-slice packaging, and unresolved broader cumulative-carrier drift |
...
| Parallel composition | `true residual drift` | Concurrent child execution is real, and TASK-435 now preserves branch-local execution-record carriers plus enclosing aggregate trace/effect/obligation/provenance state for `Par`, but the current runtime still does not justify claiming a fully explicit interleaving machine or total closure of every helper-backed concurrent aggregation latitude over `Ω` / `π` / `T` / `ε̂`. | future runtime/interpreter follow-on | keep `Par` called out explicitly in closeout and drift-prevention material |
| Capability / policy / obligation workflows | `true residual drift` | Runtime policy/obligation hooks exist, and TASK-433 now packages one authoritative terminal-facing execution-record slice for obligations/provenance/trace/effects on the main execution path, but the accepted cumulative carriers are still not yet uniformly realized across all runtime-owned paths. | future runtime/interpreter follow-on | preserve as an open carrier-realization item |
| Spawn / completion observation contracts | `true residual drift` | Spawn/control authority is directly realized, and TASK-406/TASK-407/TASK-408/TASK-409/TASK-410/TASK-411/TASK-412 now provide a sealed retained-completion carrier, a real spawned-child execution path with automatic completion sealing, one honest retained `CompletionPayload.result`-like field, one conservative retained `CompletionPayload.effects`-like summary slice, one honest-but-terminal-visible-only `CompletionPayload.obligations`-like slice, one runtime-owned retained provenance slice, and one dedicated wait surface for the first sealed retained record. Residual drift remains because the current realization is still only partial/cooperative: it does not yet provide exact effect transport, exact full obligations parity, full `CompletionPayload` contents, or stronger lifecycle ownership beyond the current detached/cooperative substrate. | future runtime/interpreter follow-on | keep separate from generic carrier work so completion observation does not disappear into broader cleanup |
...
| Authoritative cumulative `Ω` / `π` / `T` / `ε̂` packaging | `true residual drift` | TASK-433 now lands the first runtime-owned `ExecutionRecord` packaging slice on the main execution path, but the current runtime still only partially packages or distributes these carriers overall. Because the accepted five-layer story treats them as cumulative semantic state, the remaining gap is not merely editorial. | future runtime/interpreter follow-on | sequencing; capability / policy / obligation workflows; indirectly `Par` |
| Retained completion-payload-style observation | `true residual drift` | TASK-406 adds one explicit sealed/write-once retained runtime carrier for terminal completion-style observation, TASK-407 adds the missing runtime-owned child execution substrate so automatic retained completion recording occurs on a real spawned-child completion lifecycle path keyed by `workflow_type`, TASK-408 preserves the direct terminal child `Result<Value, ExecError>` inside that retained record, TASK-409 adds a conservative retained effect summary (`effects.terminal_upper_bound` plus conservative `effects.reached_upper_bound`), TASK-410 adds one honest retained obligations slice (terminal-visible local pending obligations plus terminal-visible active-role pending/discharged state), TASK-411 adds one runtime-owned retained provenance slice (child identity plus spawn lineage), and TASK-412 adds a dedicated wait surface for the same sealed retained record. The implementation still remains intentionally minimal and does not yet expose exact effect transport, exact full `CompletionPayload.obligations` parity, or full `CompletionPayload` contents. | future runtime/interpreter follow-on | spawn / completion observation contracts |
| Full helper-backed concurrent cumulative-state aggregation for `Par` | `true residual drift` | TASK-435 now goes beyond `join_all(...)` plus `Value::List(...)` by rebuilding the enclosing `Par` execution record from branch-local execution-record snapshots, but the runtime still does not yet justify claiming full closure of the entire accepted helper-backed concurrent aggregation contract. | future runtime/interpreter follow-on | parallel composition |
...
| Parallel composition | Surface, IR, big-step, and small-step `Par` structure are accepted, including helper-backed all-success aggregation as the semantic target. Runtime evidence now confirms concurrent child execution, cloned branch-local `Context`, shared runtime registries, and TASK-435's branch-local execution-record aggregation into the enclosing `Par` record for trace/effect/obligation/provenance state, rather than only direct successful `Value::List(...)` collation. | `true residual drift`: the runtime still does not justify claiming full helper-backed cumulative-state aggregation over `Ω` / `π` / `T` / `ε̂`, nor an equally explicit interleaving/branch-step realization. |
...
| Spawn / completion observation contracts | Surface, IR, big-step, and small-step inputs are accepted, including the upstream completion-observation contract. Runtime evidence confirms spawn-time control authority and pause/resume/kill/check-health boundaries through `ControlLinkRegistry`; TASK-406 adds one explicit retained completion-observation carrier in `ash-interp` with write-once/sealed semantics; TASK-407 adds a runtime-owned child workflow registry plus real spawned-child execution path keyed by `workflow_type`, allowing the runtime to automatically seal retained completion from the child lifecycle without regressing supervisor control semantics; TASK-408 preserves direct child terminal success/error results in `RetainedCompletionRecord.result`; TASK-409 adds the first retained `effects` slice; TASK-410 adds `RetainedCompletionRecord.obligations` with terminal-visible local pending obligations plus terminal-visible active-role pending/discharged state; TASK-411 adds `RetainedCompletionRecord.provenance` with runtime-owned child identity plus spawn lineage; TASK-412 adds `RuntimeState::wait_for_retained_completion(&ControlLink)` so callers can await the first sealed retained record directly; and TASK-437 upgrades child-owned retained `effects` to exact terminal/reached effect parity from the sealed child execution record while keeping control tombstones effect-payload-free. | `true residual drift`: retained completion observation is now materially stronger, but the current carrier remains intentionally partial rather than full `CompletionPayload` contents overall: obligations remain terminal-visible subset only, provenance remains conservative, and the retained surface still stays distinct from the broader execution-record contract. This row therefore stays open independently of the broader cumulative-carrier work. |

### Current residual-gap state after TASK-400 closeout

The first runtime-side follow-on for this frozen residual set is now TASK-405, which adds a conservative authoritative runtime outcome/state classification in `ash-interp` and wires current `ExecError` plus control-link lifecycle surfaces into that public class. TASK-406 then adds an explicit retained carrier in `ash-interp`, seals retained terminal observations write-once, and removes the eager inline spawn-termination regression so control links remain useful after spawn. TASK-407 now adds the missing runtime-owned spawned-child execution substrate keyed by `workflow_type`, with a narrow child entry contract that binds the evaluated spawn `init` value into child context as `init`, and uses that real child lifecycle path to drive automatic retained completion sealing. TASK-408 then enriches that retained carrier with direct terminal child result payloads via `RetainedCompletionRecord.result: Option<Box<ExecResult<Value>>>` plus `terminal_result()`. TASK-409 adds `RetainedCompletionRecord.effects: Option<ConservativeRetainedEffectSummary>` plus `conservative_effect_summary()`, preserving `effects.terminal_upper_bound` and conservative `effects.reached_upper_bound` without claiming full trace transport. TASK-410 adds `RetainedCompletionRecord.obligations: Option<ConservativeRetainedObligationsSummary>` plus `conservative_obligations_summary()`, preserving the narrowest honest terminal-visible obligations slice. TASK-411 adds `RetainedCompletionRecord.provenance: Option<ConservativeRetainedProvenanceSummary>` plus `conservative_provenance_summary()`, preserving the narrowest honest runtime-owned child identity and spawn-lineage slice. TASK-412 adds `RuntimeState::wait_for_retained_completion(&ControlLink) -> Result<RetainedCompletionRecord, ControlLinkError>`, allowing callers to await the same sealed retained record directly without polling. TASK-433 then adds the first runtime-owned authoritative execution-record substrate on the main execution path: `ExecutionRecor... [truncated]
...
| 2026-04-05 | Opened and implemented the first runtime-side follow-on for the blocked / terminal / invalid residual item | TASK-405 adds a conservative authoritative runtime outcome/state classification in `ash-interp`, wiring current execution and control-link surfaces into one public class without claiming that cumulative carriers, retained completion payloads, or helper-backed `Par` aggregation are now closed |
| 2026-04-05 | Added the next narrow runtime-side follow-on for retained completion observation | TASK-406 adds one explicit retained completion-observation carrier in `ash-interp` via `ControlLinkRegistry`/`RuntimeState`, making terminal child completion-style state queryable after completion while still not claiming full `CompletionPayload` parity or broader cumulative-carrier closure |
