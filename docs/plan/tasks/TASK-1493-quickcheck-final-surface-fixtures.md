# TASK-1493: QuickCheck Final-Surface Fixtures

## Status: 📝 Planned

## Description

Add no-Cargo fixtures for default Arbitrary, strategy overrides, and failing/shrinking quickcheck evidence.
...
- 📝 TASK-1492: prerequisite task in this phase
...
- `fixtures/phase150-quickcheck-arbitrary-strategy/`

## Requirements

1. Add a passing default-Arbitrary property/law fixture.
2. Add a passing explicit strategy override fixture, such as sorted lists or safe expressions.
3. Add a failing fixture with shrunk counterexample and shrink trace.
4. Verify with `$ASH_UNDER_TEST test ... --format json` and no `cargo run` as final evidence.
...
strictness: clean
commands:
  - ASH_UNDER_TEST=$PWD/target/debug/ash "$ASH_UNDER_TEST" test fixtures/phase150-quickcheck-arbitrary-strategy --format json
  - cargo fmt --check
  - git diff --check
checklist:
