...
| [TASK-2013](tasks/TASK-2013-source-handler-and-handle-lowering.md) | Implement source handlers and `handle ... with` lowering | In progress — parser/AST preserves canonical clauses/origins; checked declarations retain marker/signature and concrete clause facts. Alongside the closed-empty inspection slice, TASK-2014 now admits exactly local `absorb_sleep` over `TestClock::sleep(Int) -> Int` with direct `resume(ms)`, identity `done`, literal `0`, canonical parsed Core/anchor provenance, and one root `SourceHandler` instruction authorizing its one engine-private checked-CPS handler installation/dispatch. Only same-Engine `run`/`run_file` consume its opaque token; it has no provider binding/provider frame or row-derived/general/multi-frame authority, and generic execute/V1/CLI helpers remain closed. The handler prewalk accepts plain/record ambient `do` binds, legacy tuple-ADT patterns retain positional payload typing, and top-level `true`/`false` exhaust `Bool`; none broadens handler lowering. Nonempty/open rows, ordered frame installation/TASK-1993 dispatch, providers/residuals, async timeout/cancellation, and general handlers remain excluded. |
| [TASK-2014](tasks/TASK-2014-source-handler-runtime-boundary-decision.md) | Decide the source-handler runtime boundary | In progress — Path B makes checked Core/CPS the sole production owner of admitted source and keeps every source form without validated typed lowering closed. The bounded pure fragment includes sequential nonshadowing atomic lets; duplicate lexical shadowing still fails at checked Core validation and input branches remain closed. The Engine-private host-operation driver has two one-frame provider admissions: compatible built-in `time::sleep`, and local `TestClock::sleep(Int) -> Null` with only literal or prior checked lexical `Int` delay. Separately, `run`/`run_file` admit exactly the closed-empty `absorb_sleep` handler over `TestClock::sleep(Int) -> Int`, direct `resume(ms)`, identity `done`, and literal `0`, using retained canonical parsed anchor/Core provenance, checked facts, and one root `SourceHandler` instruction in an opaque same-Engine token. That instruction authorizes its one engine-private checked-CPS handler installation/dispatch; there is no provider binding/provider frame or row-derived/general/multi-frame installation. Generic/imported/`PosixFs` declarations, all other handlers/`resume`/`done`, residual/open rows, multi-frame TASK-1993 production evidence, CLI trace/runnable handler routes, remaining routes, and missing-admission/malformed-Core/handler-trap terminal taxonomy remain open. |
