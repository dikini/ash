...
- **Current focused target:** `crates/ash-typeck/tests/task_2073_checked_module_finalization.rs` passes
  112/112; the Type-layer slice remains `partial / tested / below_spec`.
...
- **Public expression dependency closure:** Public law, policy, proof, and macro expression dependencies reject missing local/imported value callables and missing parent-scoped `Impl::operation` members before publication. Unknown/resource effect-row operation items retain
  their checker-owned non-authorizing semantics. Negative evidence is
