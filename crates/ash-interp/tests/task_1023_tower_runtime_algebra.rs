//! TASK-1023 runtime checks for public tower algebra operations.
...
fn force_act_closure(value: Value) -> Value {
    ...
    .expect("act carrier closure should force with hidden runtime ActEnv")
}
...
#[test]
fn task1023_act_tower_runtime_public_unit_preserves_act_opacity() {
...
#[test]
fn task1023_act_tower_runtime_public_bind_sequences_without_exposing_actenv() {
    let act = public_bind(
        "act",
        call("act", "unit", vec![lit(Value::Int(1))]),
        call("act", "unit", vec![lit(Value::String("done".to_string()))]),
    );
...
#[test]
fn task1023_proc_tower_runtime_public_unit_and_bind_preserve_process_carrier() {
    ...
    let bound = public_bind(
        ...
        call("proc", "unit", vec![lit(Value::String("done".to_string()))]),
    );
...
#[test]
fn task1023_workflow_tower_runtime_public_unit_and_bind_preserve_workflow_carrier() {
