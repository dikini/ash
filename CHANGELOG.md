...
|
|### Added
|- [Phase 136](docs/plan/PLAN-136-INTERFACE-LAW-SYNTAX.md): Parser support for `law` declarations inside interfaces (TASK-1360) and at module scope (TASK-1361), plus `proof` declarations inside impl blocks (TASK-1362) and at module scope (TASK-1363). The parser now accepts `law` keyword with parameters and proposition expressions both inside `interface { ... }` blocks and as top-level module items, and stores impl-scoped and module-scoped proof bodies for later verification stages.
