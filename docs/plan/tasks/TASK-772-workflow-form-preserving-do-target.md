...
5. Check `let`, `<-`, `_ <-`, and final `return` with the same SPEC-054 rules. `<-` in `do:Workflow` requires `Workflow<A>` RHS.
...
3. Write failing tests for workflow `<-` producing nested `Bind` WorkflowForm nodes.
...
6. Write regression tests proving `do:Act` and `do:Proc` still elaborate through their existing paths.
7. Implement target, artifact, checking, and elaboration changes.
8. Run focused `ash-typeck` tests.
