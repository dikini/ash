...
| [SPEC-065](SPEC-065-PROMOTED-DATA-CONSTRUCTORS-NAMED-DATA-KINDS.md) | Promoted Data Constructors and Named Data Kinds | Draft | Opt-in DataKinds-style promotion packet for runtime ADT constructors as type-level constructors, keeping promoted identities distinct from sealed-domain marker constructors and ordinary runtime constructors. |
| [SPEC-066](SPEC-066-TYPE-HOLES-PARTIAL-CONSTRUCTOR-APPLICATION.md) | Type Holes and Partial Type-Constructor Application | Draft | Explicit source `_` holes and partial constructor applications such as `Result<_, E>`, including kind/arity/ambiguity diagnostics and do-target shape elaboration without implicit currying or inversion. |
| [SPEC-067](SPEC-067-CONSTRUCTOR-KINDED-PARAMETERS-AND-HKT.md) | Constructor-Kinded Parameters and HKT | Draft | Kinded binders such as `M : * -> *`, constructor-variable application, higher-kinded interface/impl coherence, and the path from built-in do dictionaries to `Monad<K>` evidence. |
| [SPEC-068](SPEC-068-PATTERN-EXHAUSTIVENESS-CANONICALIZATION.md) | Pattern and Exhaustiveness Canonicalization | Draft | Audit-first rollout of transparent alias/projection canonicalization into pattern checking and exhaustiveness without neutral-head inversion or constructor-name leakage. |
