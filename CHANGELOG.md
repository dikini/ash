...
- Fixed Phase 67 execution-record authority drift in `ash-interp` by preventing spawned child executions from overwriting `RuntimeState::last_execution_record()` and by rebuilding `Par` execution records from branch-local recorder snapshots instead of one shared recorder. Top-level and stream-path executions now retain the authoritative parent record after spawn, while `Par` now aggregates branch-local trace, effect, obligation, and provenance carriers into the parent execution record with regression coverage for both spawn and parallel cases.
