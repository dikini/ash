...
|---|---|---|
| MOD-REAL-001/002 graph, structural children, and file/inline units | Complete | Static structural realization and file/inline parity for the callable route. |
| MOD-REAL-003/004 finalization, imports, visibility, and public export propagation | Complete | Checked interfaces preserve identity, origin, visibility, and metadata for every public declaration needed by an importer. |
| MOD-REAL-005 checked Core/CPS lowering and transport | Complete | Ordinary callable bodies and non-callable metadata carriers cross the checked route; no authority is inferred from metadata. |
| MOD-REAL-006 Engine admission and client parity | Complete | Engine-sealed checked closure reaches CLI/daemon with normalized-terminal parity. |

The long status and seam tables below are retained as historical pre-closeout snapshots. Their
intermediate `in progress`, `partial`, and `below_spec` labels are not current Phase 207 blockers.

**Historical baseline status:** Complete — planning baseline and seam inventory reconciled through
the bounded early handoffs; the detailed pre-closeout slice inventory follows below.
...
(completed TASK-2075, `partial / tested / below_spec` after Tasks 5–8's atomic paired collector,
namespace/collision, parent/member/constructor, typed notation/diagnostic, full impl-head
coherence, keyed source-drift, normalized Type-layer file/inline, generated/property,
compatibility, and authority-fence evidence). The internal
`CanonicalCollectedModuleSnapshot` may retain raw
...
## Historical live seam inventory and retained fences
...
| Graph identity and expansion | `crates/ash-parser/src/canonical_module_graph.rs::CanonicalModuleGraphResolver` publishes an all-or-nothing parser graph keyed by `ModuleKey`. TASK-2074's public `CanonicalExpandedModuleGraph` owns that graph, performs the AST-only public-macro/notation syntax prepass with deterministic provider ordering/cycles and provenance, rejects unsupported item generation and invalid notation dependencies, activates validated notation rows only in their consumer, publishes exactly one shallowly expanded record per key, retains uses/module declarations/order/per-key sidecars, and returns anchored failures atomically. Exact malformed selectors reject at parser-owned anchors, and legacy Engine paths fail closed without acquiring notation authority. | Continue TASK-2075's graph collector after its completed expansion handoff and delivered visibility/private-carrier prerequisites. Generalized mixfix use-site parsing/elaboration is outside TASK-2074. | TASK-2067 (Complete parsed graph), TASK-2074 (Complete parser handoff; broader rules partial/tested/below-spec) |
| Imports | `crates/ash-parser/src/import_resolver.rs` and Engine loader have distinct import paths; TASK-2061 adds only an in-memory explicit/group/glob resolver over checked wrappers | TASK-2068 preserves its bounded tested planners/binders. TASK-2070 completed only the direct self alias; TASK-2072 owns complete parsed grammar, edge/cycle/precedence/duplicate semantics and staged `pub use`; TASK-2073 alone finalizes exports before TASK-2069 fences Engine readers. | TASK-2070 (Complete partial handoff), TASK-2072 (Complete parsed-import/binding handoff, partial/tested/below-spec), TASK-2073 (In progress, partial/tested/below-spec), TASK-2069 (In progress, partial/tested/below-spec) |
| Collection and binding | `canonical_provisional_module_scopes.rs` remains the bounded TASK-2068/TASK-2070 compatibility route. TASK-2075 now atomically publishes paired internal/name-only views across the expanded graph, with exhaustive namespace disposition, parent-aware collisions and member/constructor identities, internal-only impl facts, typed notation keys and diagnostics, full alpha-normalized impl-head coherence within the current/lexical interface scope, late-sibling failure rejection, keyed source-drift revalidation, normalized file/inline Type-layer projection, generated/property coverage, and a complete later-layer authority fence; imported interface identities fail closed pending TASK-2072 binding. | Preserve the compatibility facade. TASK-2072 consumes only the non-authorizing name view; TASK-2073 consumes the internal snapshot plus staged bindings. | TASK-2071 (Complete contract), TASK-2075 (Complete paired collection handoff, partial/tested/below-spec), TASK-2072 (Complete parsed-import/binding handoff, partial/tested/below-spec), TASK-2073 (In progress, partial/tested/below-spec) |
| Summaries | TASK-2060 adds the V1 Core `PublicModuleInterface` schema with public binding validation and V1--V8 summary compatibility; TASK-2066 adds a bounded wrapper; TASK-2061 stores only that wrapper. Typed summary identities, aliases/re-exports, source origins, and complete closure remain unlinked and Engine-private metadata/scanners are unchanged | TASK-2073 completes typed namespace linkage/export closure in the Type layer, then TASK-2069 transports only checked artifacts while retiring/fencing Engine scanners | TASK-2068 (Complete foundation), TASK-2073 (In progress, partial/tested/below-spec), TASK-2069 (In progress, partial/tested/below-spec) |
| Inline modules | Parser stores ordered `ModuleBody` values and TASK-2067's canonical graph retains acquired inline `ModuleUnit` values without filesystem access. It proves complete ordered payload parity and payload-only mutation at the parser graph boundary; its transient file-source reentrancy guard remains active across inline resolution, while Engine has explicit unsupported-inline guards. | TASK-2073 proves Type-layer final-interface parity, then TASK-2069 transports full artifacts without a source-form branch after acquisition. | TASK-2068 (Complete foundation), TASK-2073 (In progress, partial/tested/below-spec), TASK-2069 (In progress, partial/tested/below-spec) |
| Lowering | TASK-2062 adds `ash_typeck::module_core_cps_lowering` over TASK-2061 facts and `ash_core::module_lowering` non-executable carriers | Lower complete checked definition bodies, preserve identities/origins, and transport the full non-sealed closure without parser/source rediscovery or callable authority | TASK-2069 (In progress, partial/tested/below-spec); TASK-2063 consumes it |
| Entry/runtime | PLAN-203 requires one Engine-owned route; TASK-2063 now has a bounded 8/8 linked/admission request and checked-CPS execution witness | TASK-2063 must extend the sealed route across the remaining supported callable/dependency forms; TASK-2064 then proves broader CLI/daemon terminal parity | TASK-2069, TASK-2063, TASK-2064 |
...
an authoritative parse is available. TASK-2057's completed denylist remains binding; TASK-2069
owns the in-progress Engine/synthesized-runner scanner and path-cache retirement or fencing, while
...
| `strip_module_metadata_non_definition_lines` in `crates/ash-engine/src/module_loader.rs` | Masks `use`/`mod` before selected metadata lowering. **Compatibility-only and rejected before ordinary project-module binding when parser-owned structure is unavailable.** | TASK-2069 (in progress) | Expanded parsed module items must feed TASK-2073's complete checked-interface route; no masked text may become semantic input. |
| `crates/ash-cli/src/test_runner/synthesized.rs::strip_synthesized_metadata_non_definition_lines` | **Compatibility-only synthesized-runner introspection scanner.** `build_runner_introspection_snapshot` uses it only after authoritative parsing fails; it produces deferred metadata and has no graph, binding, interface, lowering, admission, or execution authority. | TASK-2069 (in progress) | Replace or fence the preprocessing with the authoritative AST/module-unit carrier, or retain an explicit disagreement-only quarantine with a negative authority test. |
...
| `collect_module_exports` text supplements in `crates/ash-engine/src/module_loader.rs` | Adds compatibility metadata after partial parsing. **Parser-owned structure is preferred; fallback records are explicitly rejected before ordinary project-module binding and cannot enter the canonical closure.** | TASK-2069 (in progress) | TASK-2073's complete checked interface must collect all supported exported namespaces; no supplement scan may be semantic authority. |
| path/string-keyed Engine module caches and raw path walking | Remains in the legacy compatibility loader; canonical module execution now uses `ModuleKey` plus checked interface/Core/CPS identity. | TASK-2069 (in progress) | Canonical `ModuleKey` and checked interface/artifact identity are the sole semantic/cache keys before TASK-2063 seals admission; legacy path caches remain non-authorizing. |
...
import/runtime authority. In-progress TASK-2069 supplies the complete non-sealed Core/CPS closure plus
...
## Historical broader target rule families
...
| MOD-REAL-006 | Engine admission and CLI/daemon execution use the same linked module artifact | partial / tested / below_spec | TASK-2069 supplies the bounded non-sealed closure; TASK-2063 seals bounded admission and TASK-2064 owns broader real-program/client parity |
