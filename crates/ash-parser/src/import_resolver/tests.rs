...
fn test_visibility_complex_hierarchy() {
    ...
    let (graph, root, left, right, l1, l2, _r1, _r2, l1a, r1a) = create_complex_hierarchy_graph();
...
fn test_capability_import_with_alias() {
    ...
    assert!(matches!(&binding.kind, BindingKind::Aliased { original } if original == "fs_read"));
}
