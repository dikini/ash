...
| [SPEC-069](SPEC-069-ALPHA-VISIBLE-TOWER-ALGEBRA-AND-DO-LOWERING.md) | Alpha Visible Tower Algebra and Generalized Do Lowering | Partial MVP | DESIGN-040 alpha target for visible `Act`/`Proc`/`Workflow`/user computation algebra, full `Monad<K>` evidence-selected `do:K` bind lowering, typed computation-expression/TCIR boundary, AMIR/bytecode traceability, and OODA demotion to library/template/lint surface. |
| [SPEC-070](SPEC-070-ALPHA-RUNTIME-KERNEL-AND-OS-SURFACE.md) | Alpha Runtime Kernel and OS-Facing Execution Surface | Partial MVP | DESIGN-041 alpha runtime regime: one `RuntimeKernel` with one-shot `ash run` and local `ashd` host modes, explicit roots, definition/instance/artifact identity, admission authority, reload semantics, and same semantics across host lifetimes. |
