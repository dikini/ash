...
- [TASK-853](docs/plan/tasks/TASK-853-import-order-reexport-determinism.md): Added focused `ash-engine` import-order, re-export, idempotence, named-import leakage, and glob determinism coverage for SPEC-062 public type-computation summaries. Engine selected-summary merging now coalesces compatible same-module public type-function summary subsets without duplicating repeated imports while preserving conflicting computation facts for later validation. The new regression proves batch TypeEnv registration is order-independent for cross-summary sealed-domain and computation-head dependencies that fail under one-at-a-time registration, and verifies `pub use` keeps original `TypeComputationHeadId` values and equation order.
