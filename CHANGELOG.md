...
### Added
- [TASK-967](docs/plan/tasks/TASK-967-toolchain-metadata-and-xdg-layout.md): Completed the Ashgrove metadata/XDG launcher substrate by installing real stable `ash` and `ashgrove` launcher shims under the configured home-local bin root, dispatching through a stable user-local `.ashgrove-dispatcher` copy and typed installed-toolchain metadata with explicit `ASH_TOOLCHAIN` override, project `ash.toml` pin, then user default order, and failing closed for missing/incomplete toolchains plus symlink/path traversal targets under temporary XDG/home roots. SPEC-073 remains Draft because later Phase 127 acceptance rows still defer real source builds, release tarball production and packaged dispatcher lifecycle, authenticated URL installs, and direct fetched-cache dependency roots.
- [TASK-973](docs/plan/tasks/TASK-973-vendor-and-deployable-git-project-flow.md): Added a public CLI regression proving selected/explicit stdlib roots take precedence over auto-discovered project `vendor/ash/<package>` roots, so a locked vendored package shaped like a stdlib module cannot override the selected stdlib while ordinary locked dependency imports still resolve through the vendor namespace. TASK-973 remains partial because direct fetched-cache dependency-root discovery and broader deployment packaging are deferred.
- [TASK-972](docs/plan/tasks/TASK-972-ash-manifest-lock-git-fetch.md): Hardened `ashgrove lock` so accepted abbreviated manifest `rev` values are serialized as resolved full commit hashes in `ash.lock`, and existing lockfile `[trust]` metadata is preserved during lock rewrites. TASK-972 remains partial because direct fetched-cache module-root discovery and broader manifest metadata coverage are still deferred.
- [TASK-971](docs/plan/tasks/TASK-971-remove-cleanup-flow.md): Added the conservative `ashgrove cleanup` planner slice: `cleanup --project PATH --dry-run` reports protected project pins without deleting, `--cache` deletes only known Ash-owned cache children, `--orphans` deletes invalid toolchain directories under the XDG toolchain root, and `--old-toolchains` deletes only unprotected installed toolchains while preserving default, project-pinned, live-daemon, and running-manager toolchains. TASK-971 remains partial because broader lockfile/cache reachability analysis and interactive confirmation policy are deferred.
- [TASK-970](docs/plan/tasks/TASK-970-update-default-list-current-flow.md): Implemented the local/source/tarball `ashgrove update` selector slice: `list` and `current` now validate selector state against installed manifest/install metadata, `default <toolchain-id>` requires an installed exact id, source/tarball updates require `--to` to match the local payload identity, update without `--switch` preserves the existing default, `--switch` changes it, first update install initializes it, and regression coverage proves the old toolchain metadata is not mutated. TASK-970 remains partial because release-index/network update, real source builds, authenticated tarball URL update, and signing/trust enforcement are still deferred.
- [TASK-969](docs/plan/tasks/TASK-969-binary-tarball-install-flow.md): Hardened local binary tarball installs by validating typed manifest/install-record identity before staged publish, requiring bundled stdlib metadata and manifest presence, rejecting unsafe symlink, hardlink, absolute-path, parent-traversal, and setuid archive entries, and recording local tarball path, digest, and install time. TASK-969 remains partial because the repository release tarball producer and authenticated URL install path are still deferred.
- [TASK-968](docs/plan/tasks/TASK-968-source-install-flow.md): Hardened the fixture-backed source install flow so source installs publish through staged collision checks, reject same-id metadata conflicts, leave identical reinstalls deterministic, and record source URL, source revision, build profile, target triple, dirty/unidentified override state, reproducibility, and install time. TASK-968 remains partial because real cargo builds from source checkout/archive and installed `ash` launcher-to-stdlib routing are still deferred.
