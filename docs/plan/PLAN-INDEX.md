...
| [TASK-2026](tasks/TASK-2026-sealed-forward-sleep-handler-provider-production.md) | Seal one `forward_sleep` handler-plus-provider checked-CPS production slice | In progress — the exact canonical row-annotated local `sleep(0) → wake(ms)` fixture now seals same-Engine source/Core/anchor provenance, checked facts, one exact registered `wake` binding, and explicit outer Provider(`wake`) then inner SourceHandler(`sleep`) instructions. The driver reverse-scans innermost-first, returns `Int(0)`, and keeps rows/generic execution/CLI/trace/all other forms closed. Focused timeout/cancellation and cancellation-wins-tie evidence for this composition remains required before completion. |
| [TASK-2013](tasks/TASK-2013-source-handler-and-handle-lowering.md) | Implement source handlers and `handle ... with` lowering | In progress — parser/AST preserves canonical clauses/origins; checked declarations retain marker/signature and concrete clause facts. Alongside the closed-empty inspection slice, TASK-2014 admits exactly local `absorb_sleep` with one root `SourceHandler` instruction, while TASK-2026 in progress consumes exactly the row-annotated `forward_sleep` facts under separately sealed outer Provider(`wake`) then inner SourceHandler(`sleep`) instructions. Neither route grants frames from rows or generalizes handler lowering; generic execute/V1/CLI helpers remain closed. The handler prewalk accepts plain/record ambient `do` binds, legacy tuple-ADT patterns retain positional payload typing, and top-level `true`/`false` exhaust `Bool`; none broadens handler lowering. General continuation/residual/open-row/multi-clause semantics, arbitrary provider chains, and async-control evidence for `forward_sleep` remain excluded. |
| [TASK-2014](tasks/TASK-2014-source-handler-runtime-boundary-decision.md) | Decide the source-handler runtime boundary | In progress — Path B makes checked Core/CPS the sole production owner of admitted source and keeps every source form without validated typed lowering closed. The Engine-private host-operation driver has the existing one-frame provider slices and closed-empty `absorb_sleep` source-handler slice. TASK-2026 now adds an in-progress exact two-instruction `forward_sleep` composition with same-Engine provenance, a registered `wake` binding, and explicit outer Provider(`wake`) then inner SourceHandler(`sleep`) installation; normal return and pre-dispatch guards exist, while its timeout/cancellation evidence is open. Rows never install frames, and generic/imported declarations, all other handler forms, CLI trace/runnable handler routes, remaining terminal taxonomy, and general multi-frame semantics remain closed. |
