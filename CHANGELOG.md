...
- **Phase 70: Capability Call Dispatch Split** - Completed all tasks (TASK-463 through TASK-470). Key changes: (1) Split provider lookup from action dispatch in the operational semantics; (2) Core `Workflow::Act` now carries explicit `provider_name` and `action_name` fields instead of overloaded single name; (3) New surface syntax support for symbolic capability calls (`capability(args)`, `capability(args) when guard`) and explicit provider:action calls (`provider:action(args)`, `provider:action(args) when guard`); (4) Updated provider trait to provider-local `execute(action_name, args)` dispatch; (5) Resolver now produces `(provider, action)` target pairs for symbolic capability names; (6) All specs updated (SPEC-001, SPEC-002, SPEC-003, SPEC-004, SPEC-010, SPEC-017, SPEC-025) with split dispatch contract; (7) Examples and documentation updated to show new call sugar patterns; (8) Legacy `act ...` forms remain compatible but lower to same split contract. This completes the operational call model unification across parser, lowering, interpreter, engine, and spec surfaces.
