...
| [TASK-974](tasks/TASK-974-ashgrove-closeout-acceptance.md) | Close out SPEC-073 with acceptance matrix and broad verification | 8 | ✅ Complete closeout report; Phase 127 remains partial |
...
Current implementation note: the source install slice builds real local source roots from an isolated cache copy with an external Cargo target dir, passes `--locked` when `Cargo.lock` exists, rejects git-like roots when `HEAD` or dirty status cannot be determined, leaves clean no-lock source fixtures clean, records source URL/revision/build profile/target triple plus dirty and unidentified-source override state, adds dirty tree digests to metadata and dirty override toolchain IDs, copies source stdlib package metadata into installed `lib/ash/std/ash.toml`, and preserves existing bundled runtime stdlib support modules under `lib/ash/std/src/runtime/`. Prepared source-shaped directory installs still cover archive-shaped local inputs, and staged publish/collision checks keep identical reinstalls deterministic while rejecting metadata conflicts. TASK-968's source-install-owned slice is complete; Phase 127 remains partial because source archive release metadata and concrete runtime-support payload metadata are owned by Phase 128 TASK-977/TASK-978 after the TASK-976 audit gate.
