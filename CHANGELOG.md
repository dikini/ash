...
### Added
- [TASK-967](docs/plan/tasks/TASK-967-toolchain-metadata-and-xdg-layout.md): Completed the Ashgrove metadata/XDG launcher substrate by installing real stable `ash` and `ashgrove` launcher shims under the configured home-local bin root, dispatching through a stable user-local `.ashgrove-dispatcher` copy and typed installed-toolchain metadata with explicit `ASH_TOOLCHAIN` override, project `ash.toml` pin, then user default order, and failing closed for missing/incomplete toolchains plus symlink/path traversal targets under temporary XDG/home roots. SPEC-073 remains Draft because later Phase 127 acceptance rows still defer packaged dispatcher lifecycle, authenticated URL installs, and direct fetched-cache dependency roots.
