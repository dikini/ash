...
**Architecture:** Phase 109 is a Tier 0 substrate phase. `ash-parser` preserves ordinary type declarations as surface module items. `ash-core` owns canonical type identities and `ModuleSemanticSummary` carriers. `ash-engine` builds/transports/imports/exports summaries without owning type semantics. `ash-typeck` consumes summaries with two-pass declaration, validation, and representation exposure. Legacy source-snippet type scanning is removed or fenced.
...
| [TASK-781](tasks/TASK-781-current-type-pipeline-audit-and-semantic-summary-gate.md) | Audit current type pipeline and freeze the semantic-summary implementation gate | Docs/Substrate | 4 | 📝 Planned |
| [TASK-782](tasks/TASK-782-modulefile-ordinary-type-declaration-surface-integration.md) | Parse ordinary `type` declarations as normal ModuleFile items | Parser/Substrate | 6 | 📝 Planned |
| [TASK-783](tasks/TASK-783-core-canonical-type-ids-and-module-semantic-summary-carriers.md) | Add core-owned canonical type IDs and ModuleSemanticSummary carriers | Core/Substrate | 8 | 📝 Planned |
| [TASK-784](tasks/TASK-784-surface-to-core-type-metadata-lowering-and-source-anchors.md) | Lower surface type metadata into core summaries with source anchors | Core/Parser | 6 | 📝 Planned |
| [TASK-785](tasks/TASK-785-engine-summary-builder-and-export-collection-from-modulefile.md) | Build engine export summaries from parsed ModuleFile/core summaries | Engine/Substrate | 8 | 📝 Planned |
| [TASK-786](tasks/TASK-786-import-pub-use-glob-visibility-and-opacity-summary-rules.md) | Implement import/pub-use/glob summary transport with visibility and opacity | Engine/Type | 7 | 📝 Planned |
| [TASK-787](tasks/TASK-787-typeenv-two-pass-registration-from-semantic-summaries.md) | Consume semantic summaries in TypeEnv with two-pass registration | Type/Substrate | 8 | 📝 Planned |
...
Remaining after TASK-780: 68 hours.
...
- TASK-785 builds and exports summaries from ModuleFile/core summaries across `check_module_file`, `collect_module_exports`, `load_ordinary_file`, `parse_file`, `parse_workflow_source_with_imports`, and runtime stdlib type discovery entry points.
- TASK-786 applies named import, glob import, `pub use`, visibility, and opacity rules.
...
- TASK-787 consumes summaries through TypeEnv two-pass declaration/validation/exposure, including canonical identity-aware keys or alias-to-identity bindings and explicit placeholder states.
...
10. docs/spec index, PLAN-INDEX, task statuses, and CHANGELOG are reconciled.
...
- [ ] Ordinary type declarations are parsed as ModuleFile definitions.
- [ ] Core-owned ModuleSemanticSummary or equivalent exists.
- [ ] Public ordinary type identities export/import through summaries.
- [ ] TypeEnv consumes summaries using two-pass registration.
- [ ] Private representations do not leak downstream.
- [ ] Constructors are imported/exposed only when representation visibility allows.
...
- [ ] Docs/changelog/status are reconciled.
