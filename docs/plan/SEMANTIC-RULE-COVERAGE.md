...
- **Role-row closure:** Public effect-row role items now apply staged local, imported, and qualified visibility checks while retaining roles as minimum namespace metadata only.
...
- **Updated focused target:** The qualified namespace, implementation-call, where-bound, transitive-row, callable-whole-row, role-row, and binding-metadata RED/GREEN slices bring the focused TASK-2073 target to 87/87.
...
- **Role-row evidence:** Positive `TEST-MOD-REAL-003-TASK-2073-PUBLIC-IMPORTED-ROLE-PUBLIC-DEPENDENCY`; negative `TEST-MOD-REAL-003-TASK-2073-PUBLIC-EFFECT-ROW-PRIVATE-ROLE-DEPENDENCY` and `TEST-MOD-REAL-003-TASK-2073-PUBLIC-IMPORTED-ROLE-PRIVATE-DEPENDENCY` reject private local/imported role paths atomically. Role declarations remain minimum metadata; no role authority, persistence, or runtime semantics are added.
...
- **Activation and implementation evidence:** `crates/ash-typeck/tests/task_2073_checked_module_finalization.rs` passes 87/87. Existing positive and negative evidence remains as previously recorded; the qualified namespace, where-bound, transitive-row, callable-whole-row, role-row, and binding-metadata evidence above extends the public implementation closure slice. These are tests, not proof or downstream execution/client parity.
