...
Define `Arbitrary<T>` as default strategy evidence and document/test coherence laws between `arbitrary`, `gen`, and `shrink`.
...
1. Add `Arbitrary<T>` with `arbitrary() -> Strategy<T>`, `gen(Int, Int) -> List<T>`, `shrink(T) -> List<T>` or the audited live-compatible staged equivalent.
2. Document `arbitrary_gen_coherent` and `arbitrary_shrink_coherent` laws.
3. Add tests or fixtures proving `gen`/`shrink` use the same strategy source for a sample type.
4. Do not introduce multiple default `Arbitrary<T>` instances for the same type.
...
strictness: clean
commands:
  - cargo test -p ash-cli arbitrary
  - cargo fmt --check
  - git diff --check
checklist:
