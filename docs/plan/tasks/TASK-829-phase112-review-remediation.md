...
- Code changed: `crates/ash-typeck/src/type_env.rs` now uses a per-alias canonical-var bridge instead of hashing canonical variable names into synthetic `TypeVar` ids, preserves unregistered nominal canonical origins through transparent alias expansion, only expands aliases when the canonical origin matches the registered alias identity, and moves fallback identities into a non-colliding synthetic crate/module space.
