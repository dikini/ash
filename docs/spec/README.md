...
| [SPEC-056](SPEC-056-FIRST-CLASS-WORKFLOW-CARRIER.md) | First-Class Workflow Carrier | Draft | `Workflow<A>` as a contract-indexed `Proc<A>` carrier with a blocking `WorkflowForm`/projection/obligation semantic gate, Monad-shaped `workflow` operations, coverage/evidence relations, `requires`/`ensures`, delayed lower Proc/Act coverage obligations, `do:Workflow`, and `[...]: Workflow` comprehension integration |
