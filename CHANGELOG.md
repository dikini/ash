...
- Pure Functions Phase: Reviewed and fixed specification/plan issues in DESIGN-020, SPEC-027, SPEC-028, and PLAN-023. Addressed critical issues: (1) added TASK-P0a for parser/module model reconciliation, (2) added function type syntax coverage in Type enum, (3) fixed AST alignment notes (removed bogus Workflow::Block reference), (4) added explicit fn name resolution/import/export tasks, (5) documented contract lowering boundary, (6) strengthened milestone acceptance criteria with 28 AC items plus failure mode tests, (7) revised effort estimate (95→120 hours) and fixed dependency graph (critical path now D2→D3, not D3→D2). Added explicit notes that fn support is cross-cutting. Remaining open questions: panic keyword vs function (decision needed before A2/A6a), type model alignment between Type::Fn (SPEC-027) and Type::Fun (SPEC-003).
