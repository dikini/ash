...
**Status:** ✅ Complete / backlog promoted
...
| [TASK-887](tasks/TASK-887-promoted-data-constructors-and-named-data-kinds-packet.md) | Promote SPEC-065/PLAN-114 for promoted data constructors and named data kinds | Future Spec Packet | 4 | ✅ Complete |
| [TASK-888](tasks/TASK-888-type-holes-and-partial-type-constructor-application-packet.md) | Promote SPEC-066/PLAN-115 for type holes/wildcards and partial type-constructor application | Future Spec Packet | 4 | ✅ Complete |
| [TASK-889](tasks/TASK-889-constructor-kinded-parameters-and-hkt-packet.md) | Promote SPEC-067/PLAN-116 for constructor-kinded parameters, HKT, and user-defined unary computation abstractions | Future Spec Packet | 4 | ✅ Complete |
| [TASK-890](tasks/TASK-890-pattern-exhaustiveness-alias-canonicalization-packet.md) | Promote SPEC-068/PLAN-117 for alias/projection canonicalization rollout into pattern checking and exhaustiveness | Future Spec Packet | 4 | ✅ Complete |
...
| Complete source-level kind checking for variables, constructors, partial applications, arity | Nominal/projection/computation-head arity is implemented; partial application and public kind binders remain deferred | SPEC-066 / SPEC-067 split the remaining surface | Partial / planned implementation |
...
| Replacement/canonicalization of stringly `Type::Associated` into projection identities | Implemented at canonical lowering/equality boundaries by SPEC-058 | SPEC-068 / PLAN-117 created; implementation planned | 📝 Planned implementation |
...
| Alias canonicalization policy for normalization and pattern matching | Equality/normalization boundaries implemented; pattern/exhaustiveness explicitly out of Phase 110 | SPEC-068 / PLAN-117 created; implementation planned | 📝 Planned implementation |
...
- D1: Do not reinterpret sealed-domain marker constructors as promoted ADT/runtime constructors. SPEC-065 chooses an opt-in promoted-constructor model.
- D2: Type holes/partial applications are distinct from HKT binders. SPEC-066 owns hole terms and partial-application elaboration; SPEC-067 owns constructor-kinded binders/interfaces.
- D3: User-defined `Monad<M>` or `do:Result<_, E>` requires both SPEC-066 and SPEC-067-class substrate; existing Act/Proc dictionaries remain the bridge until then.
- D4: Pattern/exhaustiveness canonicalization in SPEC-068 must start with an audit of live pattern/exhaustiveness callsites; do not adopt TypeEnv canonicalization by search-and-replace.
- D5: Future packets now have concrete SPEC/PLAN/TASK ranges; implementation still starts with the audit gates in PLAN-114 through PLAN-117.
...
TASK-887 through TASK-890 are now complete as documentation/spec/plan activation tasks. Feature implementation remains planned in PLAN-114 through PLAN-117 and must start with the corresponding audit gates before Rust changes.
