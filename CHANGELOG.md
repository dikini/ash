...
### Added
- [TASK-972](docs/plan/tasks/TASK-972-ash-manifest-lock-git-fetch.md) and [TASK-973](docs/plan/tasks/TASK-973-vendor-and-deployable-git-project-flow.md): Hardened Phase 127 vendored dependency module resolution so explicit `ASH_DEP_ROOTS` vendor roots or package roots cannot bypass `ash.lock` package-name and full-commit validation, while projects without `vendor/ash/` are not forced to carry `ash.lock`. Added `ash run src/main.ash:main` parity coverage for malformed commits, unlocked vendor packages, and top-level modules inside locked packages. Direct fetched-cache module-root discovery remains deferred.
