...
related-plan-tasks: [TASK-405, TASK-406, TASK-407, TASK-433]
...
The first concrete runtime-side follow-on for the frozen MCE-007 true residual drift set is now in progress/completed via [TASK-405](../../plan/tasks/TASK-405-authoritative-runtime-outcome-state-classification.md) through [TASK-435](../../plan/tasks/TASK-435-par-runtime-aggregation-realization.md).
...
This means MCE-008 should now treat runtime-state classification as the first implemented cleanup/follow-on step after the MCE-007 closeout corpus, while also recording that TASK-433 has landed the first runtime-owned authoritative execution-record substrate in `ash-interp`: `ExecutionRecord` / `ExecutionRecorder`, `RuntimeState::last_execution_record()`, and direct terminal semantic projection through `project_workflow_outcome()` / `project_completion()`. TASK-435 then extends that substrate through `Workflow::Par`: branch-local execution records are now preserved per branch and the enclosing `Par` execution record is rebuilt from branch-local trace/effect/obligation/provenance snapshots rather than one shared recorder. TASK-437 now adds one bounded retained-completion parity improvement on top of the earlier retained slices: child-owned retained completions preserve exact `effects` parity from the sealed child execution record, while obligations/provenance remain on their prior honest subset/conservative classifications. The broader runtime cleanup agenda remains open because this is still not full uniform carrier packaging, full retained-completion parity, or total `Par` aggregation closure.

TASK-406 now provides the retained completion-observation carrier slice in `ash-interp`: a minimal retained terminal record keyed by control target and exposed through `RuntimeState`. [TASK-407](../../plan/tasks/TASK-407-spawned-child-execution-substrate-and-completion-sealing.md) then adds the missing spawned-child execution substrate: `RuntimeState` now owns a narrow child-workflow registry keyed by `workflow_type`, `Workflow::Spawn` can launch a real child execution path when such an entry is registered, and that child lifecycle now drives automatic retained completion sealing honestly while still preserving useful live supervisor control authority immediately after spawn. [TASK-408](../../plan/tasks/TASK-408-richer-retained-completion-payload-contents.md) now enriches that retained carrier with one honest `CompletionPayload.result`-like slice by preserving `RetainedCompletionRecord.result: Option<Box<ExecResult<Value>>>` plus the `terminal_result()` accessor for child-owned completions while keeping control tombstones distinct as `result: None`. [TASK-409](../../plan/tasks/TASK-409-retained-completion-effect-summary-contents.md) then adds the first retained `effects` slice, and [TASK-437](../../plan/tasks/TASK-437-retained-completion-parity-follow-on.md) upgrades that child-owned retained `effects` slice from conservative workflow-form upper bounds to exact terminal/reached effect parity projected from the sealed child execution record. [TASK-410](../../plan/tasks/TASK-410-retained-completion-obligations-contents.md) now adds one honest `CompletionPayload.obligations`-like slice based on terminal-visible local pending obligations plus terminal-visible active-rol... [truncated]
