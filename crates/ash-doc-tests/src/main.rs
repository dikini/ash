...
impl CodeBlock {
    fn should_test(&self) -> bool {
        ...
        if self.lang == "rust" || self.lang == "rs" {
            !self.info.contains(&"ignore".to_string()) && !self.info.contains(&"no_run".to_string())
        } else {
            false
        }
    }
...
fn main() -> Result<()> {
    ...
    let compile_only: Vec<_> = all_blocks
        .iter()
        .filter(|b| b.should_compile_only())
        .collect();
    let ignored: Vec<_> = all_blocks
        .iter()
...
fn test_block(block: &CodeBlock, temp_dir: &Path) -> Result<TestResult> {
    ...
    let binary_file = temp_dir.join(format!(
        "test_{}_{}",
        sanitize_filename(&block.file.file_name().unwrap().to_string_lossy()),
        block.line
    ));
