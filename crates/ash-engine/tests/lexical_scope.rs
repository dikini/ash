...
async fn variables_example_scope() {
    ...
    assert!(result.is_ok(), "workflow should execute: {:?}", result.err());
...
async fn variables_example_if_scope() {
    ...
    assert!(result.is_ok(), "workflow should execute with true: {:?}", result.err());
    assert_eq!(result.unwrap(), ash_core::Value::Int(1));
    ...
    let result = engine.execute_with_input(&workflow, input).await;
    assert!(result.is_ok(), "workflow should execute with false: {:?}", result.err());
