...
#[test]
fn test_workspace_symbols_finds_matches() {
    let dir = tempfile::tempdir().expect("tempdir");
    let root = dir.path().to_str().expect("path");
    let mut f = std::fs::File::create(dir.path().join("one.ash")).expect("create");
    f.write_all(b"fn helper() -> Int { 1 }\n").expect("write");
...
#[test]
fn test_workspace_symbols_empty_query_honest() {
    let dir = tempfile::tempdir().expect("tempdir");
    let root = dir.path().to_str().expect("path");
    let mut f = std::fs::File::create(dir.path().join("one.ash")).expect("create");
    f.write_all(b"fn helper() -> Int { 1 }\n").expect("write");
    ...
    let text = extract_text(&result);
    assert!(
        text.contains("Workspace symbols not yet implemented") || text.contains("0 symbol(s)"),
