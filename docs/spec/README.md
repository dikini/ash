...
| SPEC-012 | Imports | Historical/bounded V1 | Import context; target executable import resolution is owned by SPEC-103 |
...
| [SPEC-103](SPEC-103-MODULE-REALIZATION-AND-OPERATIONAL-SEMANTICS.md) | Module Realization and Operational Semantics | Draft target-state | Complete AST-owned source-module semantics: file/inline parity, interfaces, import/visibility binding, Core/CPS artifacts, Engine linking, and client parity; it amends only the target module route while retaining SPEC-057/062 bounded-summary authority. |
