...
| [TASK-1011](tasks/TASK-1011-phase-76b-final-remediation-and-design022-023-planning.md) | Remediate final Phase 76B review blockers, reconcile narrow-slice status, and plan DESIGN-022/023 completion follow-on work | [PLAN-024](PLAN-024-ASH-TEST-RUNNER-V1.md), DESIGN-022/023, SPEC-077, PLAN-127 | 6-8 | ✅ Complete |
...
world-state metadata. TASK-514 added exact finite generated property execution and deterministic explicit
...
verification, smoke evidence, and residual limitations. TASK-1011 remediated final review
blockers: obligation lifecycle rows now evaluate explicit finite lifecycle world state before
passing, uncapped bounded-int domains defer before range materialization, and synthesized
kind/tag filters plus fail-fast apply to structured synthesized results. Raw-source recognition
and incomplete metadata defer with explicit skip output instead of reporting success; the
user-facing CLI still lacks a live checked/lowered snapshot producer for ordinary source files.
...
richer domain families, and broader synthesized execution remain deferred to Phase 132.
...
Phase 112 implements SPEC-D from DESIGN-034. It introduces the internal total normalizer and normalize-and-compare definitional equality core on top of Phase 109 semantic summaries, Phase 110 canonical type-expression/projection identities, and Phase 111 sealed-domain metadata. The phase uses internal fixture equation tables and hand-constructed canonical terms because public `type fn` syntax is deliberately deferred to SPEC-E.

Boundary note: Phase 112 is normalizer/equality substrate only. It does not add source `type fn` syntax, equation parsing, public type-function summary export/import, recursive associated type-family computation, proposition solving, or new projection syntax. Neutral computation heads remain non-inverting; ordinary nominal constructor unification remains compatible with the current typechecker.
...
**Track C (Reduction Semantics):** 17h. Implement closed reduction, open neutral/stuck forms, partial prefix normalization, aliases, and neutral/rigid projection argument normalization.
**Track D (Equality and Forcing Points):** 18h. Add structured definitional equality, enforce non-inversion under neutral computation heads, and adopt the API at named `TypeEnv` forcing points only.
**Track E (Diagnostics and Closeout):** 16h. Add diagnostics/non-interference coverage, reconcile docs/status/changelog, and reserve the usual post-review remediation slice.
...
| [TASK-832](tasks/TASK-832-parser-surface-for-type-functions.md) | Parser surface for type functions | 6 | ✅ Complete |
| [TASK-833](tasks/TASK-833-core-type-function-equation-carriers.md) | Core type-function/equation/result-expression carriers | 5 | ✅ Complete |
...
| [TASK-842](tasks/TASK-842-phase113-review-remediation.md) | Phase 113 review remediation | 6 | ✅ Complete |
...
**Status:** ✅ Complete (TASK-871 through TASK-884 complete)
...
| [TASK-875](tasks/TASK-875-typeenv-proposition-environment.md) | TypeEnv proposition environment, obligation generation, and canonical lowering | 7 | ✅ Complete |
| [TASK-876](tasks/TASK-876-normalized-equality-disequality-solver.md) | Normalized equality and conservative constructor-head disequality solver | 8 | ✅ Complete |
| [TASK-877](tasks/TASK-877-interface-bound-proposition-solving.md) | Interface-bound proposition solving without broadening impl search | 5 | ✅ Complete |
...
| [TASK-884](tasks/TASK-884-phase116-review-remediation.md) | Phase 116 review remediation | 6 | ✅ Complete |
...
Phase 122 promotes DESIGN-040 and DESIGN-041 into an implementation-grade alpha packet. It makes visible `Monad<K>` evidence and tower algebra the construction authority for `Act`, `Proc`, `Workflow`, and user/library monads; implements full generalized `do:K` bind lowering through selected evidence; introduces a traceable TCIR/AMIR/bytecode execution substrate; and defines one RuntimeKernel with two host modes, `ash run` and local daemon control through `ash daemon ...`. TASK-920 completed the hard pre-implementation audit gate and patched downstream exact evidence commands before Rust implementation work. TASK-921 added the public tower stdlib manifest and no-magic intrinsic mapping. TASK-922 added selected Monad evidence method-body/shim carriers for return-only lowering. TASK-923 generalized `<-` lowering now applies selected method bodies as parameterized closures, hardens `Result<_, E>` evidence matching, and proves interpreter plus engine-monomorphization execution. TASK-924 keeps hidden `ActEnv` and process/runtime identity carriers non-denotable while preserving explicit `proc::from_act`, `workflow::from_proc`, and `workflow::from_act` lift boundaries. TASK-925 added the TCIR computation-expression carrier and typed do attachment point with source/evidence/tower/failure/lift/workflow provenance. TASK-926 added the minimal AMIR and bytecode logical schema, stable sections, TCIR/source provenance bridge, and verifier/debug contract without source reparsing. TASK-927 added core RuntimeKernel identity carriers for host modes, roots, profile/config selection, cache keys, definitions, artifacts, workflow instances, process trees, provider registry inventory, explicit admission grants, and the typed embedded-engine relationship. TASK-928 routed one-shot `ash run` through RuntimeKernel reporting. TASK-929 added the local daemon control plane. TASK-930 preserves OODA as ordinary library/template/lint compatibility material and keeps alpha AMIR/bytecode free of OODA-specific primitive roots. TASK-931 maps SPEC-069 A69-1 through A69-12 and SPEC-070 A70-1 through A70-8 to focused evidence and explicit limitations without marking TASK-932 complete.
...
| [TASK-924](tasks/TASK-924-act-proc-workflow-opaque-carrier-alignment.md) | Align Act/Proc/Workflow opaque carrier semantics with visible algebra and explicit lifts | 10 | ✅ Complete |
...
**Status:** ✅ Complete after TASK-945 final remediation (TASK-942 through TASK-945 are post-merge remediation evidence for the Phase 123 promotion; Phase 122 remains the historical Partial MVP closeout)
**Spec:** [SPEC-069](../spec/SPEC-069-ALPHA-VISIBLE-TOWER-ALGEBRA-AND-DO-LOWERING.md), [SPEC-070](../spec/SPEC-070-ALPHA-RUNTIME-KERNEL-AND-OS-SURFACE.md)
...
| [TASK-944](tasks/TASK-944-phase123-daemon-admitted-source-config-remediation.md) | Final daemon admitted-source and config identity remediation | 6 | ✅ Complete |
| [TASK-945](tasks/TASK-945-phase123-daemon-local-control-security-remediation.md) | Final Phase 123 remediation | 4 | ✅ Complete |

**Decision gates:**
- D1: SPEC-069/SPEC-070 current status surfaces are promoted only after every formerly deferred acceptance row has concrete Phase 123 successor evidence and TASK-942/TASK-943/TASK-944/TASK-945 post-merge remediation audit status is represented honestly.
...
| [TASK-968](tasks/TASK-968-source-install-flow.md) | Implement source install path and installed-stdlib root use | 14 | ✅ Complete source-install-owned slice; Phase 128 owns deferred metadata rows |
...
**Status:** ✅ Complete; SPEC-074 Accepted/Implemented
...
Phase 129 is a targeted amendment to the SPEC-073 Implemented MVP. It separates source-root payload identity from local checkout state so ignored files such as `.agents/status/dashboard.json`, nested `target/` directories, worktrees, and caches cannot falsely abort `ashgrove install --from source`, while nonignored source payload mutations and source-archive attestation failures remain fail-closed.
...
| [TASK-1001](tasks/TASK-1001-matching-semantics-audit-gate.md) | Audit all pattern-use callsites and replace fail-closed downstream verification guards | 6 | ✅ Complete |
| [TASK-1002](tasks/TASK-1002-type-aware-irrefutable-pattern-api.md) | Add shared type-aware irrefutable pattern API | 8 | ✅ Complete |
| [TASK-1003](tasks/TASK-1003-let-and-block-let-irrefutable-enforcement.md) | Enforce irrefutable patterns for pure block/core let binders | 8 | ✅ Complete |
| [TASK-1004](tasks/TASK-1004-workflow-and-operational-binder-irrefutable-enforcement.md) | Enforce irrefutable patterns for workflow/observe/spawn/split/loop binders | 10 | ✅ Complete |
...
- D2: Every pass row is backed by evaluated target/oracle metadata or real world execution.
...
|---|---|---|
| [TASK-1359](tasks/TASK-1359-add-eq-interface.md) | Add `Eq` interface to `std::algebra` | Complete |
| [TASK-1360](tasks/TASK-1360-parser-law-in-interfaces.md) | Parser: `law` keyword in interfaces | Complete |
...
| [TASK-1366](tasks/TASK-1366-typeck-law-purity-restriction.md) | Typechecker: restrict law propositions to Pure functions | Complete |
...
| [TASK-1371](tasks/TASK-1371-cli-law-opt-out.md) | CLI: `--skip-law-tests` and `--skip-law-test=<name>` | Complete |
...
**Plan:** [PLAN-137: Rust Module Size and Discoverability Refactor](PLAN-137-RUST-MODULE-SIZE-REFACTOR.md)
...
| [TASK-1380](tasks/TASK-1380-split-ash-typeck-checking-modules.md) | Split `ash-typeck` expression/checking frontends | Complete |
| [TASK-1381](tasks/TASK-1381-split-ash-parser-surface-and-lowering.md) | Split parser surface/lowering/import resolver modules | Complete |
| [TASK-1382](tasks/TASK-1382-split-ash-engine-module-loader.md) | Split engine module loading and public engine shell | Complete |
| [TASK-1383](tasks/TASK-1383-split-ash-cli-synthesized-runner.md) | Split synthesized test runner modules | Complete |
| [TASK-1384](tasks/TASK-1384-split-ash-interp-runtime-modules.md) | Split interpreter eval/execute/runtime state modules | Complete |
| [TASK-1385](tasks/TASK-1385-split-ashgrove-and-secondary-crates.md) | Split ashgrove and secondary oversized crates | Complete |
