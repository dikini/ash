...
fn task_2073_activation_contract_is_recorded_before_finalization_implementation() {
    ...
    assert!(task.contains("checker-owned module snapshots"));
    assert!(task.contains("staged import bindings"));
    assert!(task.contains("atomic"));
    assert!(task.contains("Dedicated Ash role and policy declarations"));
    assert!(task.contains("Dynamic module loading is outside this task"));
    assert!(task.contains("static file-backed and inline acquisition remain"));
}
