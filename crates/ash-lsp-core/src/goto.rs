...
fn find_definition_span<'a>(token: &str, definitions: &'a [Definition]) -> Option<&'a Span> {
    for def in definitions {
        let name_matches = match def {
            ...
            Definition::Newtype(t) => t.name.as_ref() == token,
            Definition::EffectAlias(a) => a.name.as_ref() == token,
            Definition::EffectGroup(g) => g.name.as_ref() == token,
        ...
        if name_matches {
            return Some(match def {
                ...
                Definition::Newtype(t) => &t.span,
                Definition::EffectAlias(a) => &a.span,
                Definition::EffectGroup(g) => &g.span,
