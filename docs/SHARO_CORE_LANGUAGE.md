...
(PROGRESS)
  w makes progress if: after(w, value ≠ stuck)
```
...
## 10. Historical Scenario Sketches

Earlier drafts used source-shaped examples for customer-support, code-review, and multi-agent
research scenarios. Phase 201 removes those old source forms from productive repository
documentation. This file keeps the scenarios as historical design background only; executable Ash
examples now live under `examples/` and must use target `fn main` entry syntax.
