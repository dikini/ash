...
| TCIR/AMIR entry artifacts | checked-function artifact and computation provenance carriers | workflow artifact carriers | target Core/TCIR/AMIR computation artifacts with effect rows | Refactored to target primitive and verified by TASK-1972 | TASK-1972 |
...
| `WorkflowDef` residual carrier | removed parser/lowering carrier | workflow declaration AST | direct target function contracts and contract/evidence helpers | Removed and verified by TASK-1971 | TASK-1971 |
...
| TASK-1971 complete | Removed workflow-form parser/lowering carriers and stale tooling/test metadata: parser `WorkflowHeaderEvent`, `WorkflowDef.header_events`, `WorkflowDef.contract`, and header adaptation; surface `Definition::Proxy`; LSP proxy completion/hover/navigation/indexing; the unused typechecker `DefinitionKind::Proxy`; and the obsolete `#[workflow]` macro test. Program entries use target function metadata. | RED/GREEN Phase 201 removal gates cover parser/lowering carriers; focused LSP navigation/index tests, `ash-lsp-core` all-target checks, `ash-macros` tests/checks, typechecker regressions, and the final passing `cargo test --all` verify the surviving target forms. | Closed: the active parser surface no longer exposes proxy or workflow declaration forms and current tooling does not advertise them. |
| TASK-1972 complete | Runtime artifact construction now consumes a checked function artifact (identity, Core row, result type, source anchor, and lowered Core body) instead of synthesizing a null-bodied application entry from a text-derived row summary. TCIR/AMIR provenance records the checked function metadata and body fingerprint. | Focused real-source CLI, engine identity-mismatch/body-provenance, and cache-identity RED/GREEN regressions, plus the final passing `cargo test --all`, verify the cache/provenance path. | Closed: checked-function artifact provenance is the retained target computation model. |
