...
3. TASK-2042 carries Engine-issued admitted requests and normalized terminal envelopes through the
   daemon, with same-request `ash run` parity.
...
| [TASK-2042](tasks/TASK-2042-daemon-admitted-request-terminal-envelope-parity.md) | Daemon admitted-request transport and normalized-terminal parity | active |
...
and end-to-end tests must show identical normalized terminal results for the same admitted
supported program through `ash run`, daemon, `ash test`, and REPL. Every unsupported finite
catalogue entry must have an explicit deferred/rejected result; none may fall back. Retained Lean
