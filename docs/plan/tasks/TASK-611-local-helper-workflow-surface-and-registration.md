...
1. Extend `Program` in `surface.rs` to carry helper workflow definitions alongside the entry workflow.
...
2. Extend `Program` with `helper_workflows: Vec<WorkflowDef>`.
...
- [x] `Program` struct carries helper workflows
