...
- [TASK-963](docs/plan/tasks/TASK-963-stdlib-and-reference-callable-syntax-migration.md): Added the PLAN-121 stdlib/reference migration task so Phase 126 explicitly migrates standard-library `.ash` surfaces and the top-level `reference/` corpus to preferred callable type and pure closure syntax before closeout, while retaining legacy `Fn(...) -> ...` and `|args| => ...` only in labeled compatibility guidance.
