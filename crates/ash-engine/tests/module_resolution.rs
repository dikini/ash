...
fn write(path: &std::path::Path, contents: &str) {
    ...
    std::fs::write(path, normalize_ash_fixture(contents)).expect("write file");
}

fn normalize_ash_fixture(contents: &str) -> String {
...
async fn sibling_module_type_import_resolves() {
    ...
    let result = parse_and_check(dir.join("main.ash"));

    assert!(
        result.is_ok(),
        "sibling module resolution: expected successful parse/check, got: {:?}",
        result.err()
    );
}
...
async fn multiline_nested_ordinary_import_resolves_before_application_body() {
    ...
    assert!(
        result.is_ok(),
        "multiline ordinary import should resolve before target entry checking, got: {:?}",
        result.err()
    );
}
...
async fn semicolonless_ordinary_imports_do_not_swallow_application_body() {
    ...
    assert!(
        result.is_ok(),
        "semicolonless ordinary imports should remain line-delimited before target entry checking, got: {:?}",
        result.err()
    );
}
...
async fn nested_pub_mod_module_resolution() {
    ...
    assert!(
        result.is_ok(),
        "nested module resolution: expected successful parse/check, got: {:?}",
        result.err()
    );
}
...
async fn stdlib_module_resolution() {
    ...
    write(
        ...
        \n\
        fn main() -> Option<Int> { Some { value: 42 } }\n\
        ",
    );
    ...
    assert!(
        result.is_ok(),
        "stdlib module resolution: expected successful parse/check, got: {:?}",
        result.err()
    );
}
...
async fn ash_library_path_search_order_local_wins_over_lib() {
    ...
    write(
        ...
        \n\
        fn main() -> LocalResult { LocalResult { val: 10 } }\
        ",
    );
    ...
    let result = temp_env::async_with_vars(
        ...
        async { parse_and_check(dir.join("main.ash")) },
    )
    .await;

    assert!(
        result.is_ok(),
        "ASH_LIBRARY_PATH search order: local should win during parse/check, got: {:?}",
        result.err()
    );
}
...
async fn ash_library_path_falls_through_to_lib_dir() {
    ...
    assert!(
        result.is_ok(),
        "ASH_LIBRARY_PATH fallback: should resolve from lib dir during parse/check, got: {:?}",
        result.err()
    );
}
...
async fn ash_library_path_falls_through_to_stdlib() {
    ...
    write(
        ...
        use option::{Option, Some}\n\
        \n\
        fn main() -> Option<Int> { Some { value: 1 } }\n\
        ",
    );

    let result =
        temp_env::async_with_vars([("ASH_LIBRARY_PATH", None::<&std::ffi::OsStr>)], async {
            parse_and_check(dir.join("main.ash"))
        })
        .await;

    assert!(
        result.is_ok(),
        "stdlib fallback: should resolve `option` from builtin root during parse/check, got: {:?}",
        result.err()
    );
}
...
async fn stdlib_result_type_resolves() {
    ...
    write(
        ...
        \n\
        fn main() -> Result<Int, String> { Ok { value: 42 } }\n\
        ",
    );
    ...
    assert!(
        result.is_ok(),
        "stdlib result resolution: expected successful parse/check, got: {:?}",
        result.err()
    );
}
...
async fn pub_type_still_exports_constructor_representation() {
    ...
    write(
        ...
        "\
        use lib::{Visible}\n\
        fn main() -> Visible { Visible { value: 1 } }\n\
        ",
    );
