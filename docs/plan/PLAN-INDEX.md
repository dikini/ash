...
| [TASK-2001](tasks/TASK-2001-target-grammar-gap-and-spec-conflict-decision.md) | Realize selected target grammar and retire proxy specification conflict | In progress — authority is already selected: `GRAM-TARGET-MODULE-001 → SPEC-095b`, `TYPE-TARGET-ROW-001 → SPEC-096b/SPEC-097b`, lowering → SPEC-098c, and runtime → SPEC-099b. Parser/AST declaration slices, stable parser rejection of historical `capability`/`proxy`/top-level `yield` declarations, TypeEnv handler/newtype registration/query evidence, source-anchored newtype summary/constructor handoff, non-granting alias/group plus marked-handler export metadata, source named-import and selected public `pub use` metadata transport, and narrow imported/local alias/group row validation are recorded. Public non-generic nominal newtype named imports preserve parser-originated `NominalNewtype` kind and provider `TypeDeclId`, check their sole tuple payload, reject private imports and `Int` coercion, retain transparent ordinary aliases, and reject forged alias+constructor summaries. Normal local newtypes now retain their actual file/module identity through module-aware typecheck and Engine file/inline resolution; only direct no-module `TypeEnv` registration intentionally retains the documented fallback identity. A source-local, current-module, non-generic irrefutable `let Ctor(binding)` pattern recognizes only the declared singleton tuple constructor and binds its checked representation; wrong constructors and arities reject without nominal coercion. Direct alias/group cycles deterministically reject through `TypeEnvError::InvalidDefinition` as `Audit -> Audit`; mutual alias→group→alias rejects as `Audit -> Workflow -> Audit`; an acyclic shared-row control proves expansion-stack cleanup. No rejected or successful control grants authority/capability provenance. Normal declaration-resolved symbolic `ImplType::operation(args)` is settled by TASK-2011/TASK-2012/TASK-2017, not a TASK-2001 gate. Remaining work is specified realization: complete typed alias/group identity/expansion/discharge, full SPEC-097b diagnostic/import/versioning behavior, generic/re-export/imported and cross-module newtype patterns, `match`/`if let`/proof patterns, runtime newtype behavior, broader cross-module behavior, and runtime implementation. TASK-2014's sole user-facing gate is now decided: Path B requires strict Core/CPS cutover and closed admission, still unimplemented. |
| [TASK-2002](tasks/TASK-2002-generic-do-and-lowering-sidecar-strategy.md) | Decide generic `do` behavior and preserve target lowering sidecars | In progress — ambient `do` source/evidence/Core-Let evidence, an entry-body source anchor, successful macro- and notation-expansion origin audit metadata, deterministic named-target rejection, and deterministic all-local-callable lowered-contract artifacts are recorded. Invalid local contracts reject before an entry is published; all retained sidecars are audit/evidence-only and grant no row, runtime, monitor, provider, frame, or admission authority. Unified source/evidence/trace/diagnostic sidecars, handler boundaries, and full conformance remain open. |
| [TASK-2003](tasks/TASK-2003-return-authority-and-cps-kernel-decision.md) | Resolve `Return` authority in the CPS kernel | In progress — terminal-observation decision, checked projection, and answer-typed literal/atomic-let/typed-variable-let/boolean-conditional source-return-to-`Jump` inspection bridge are recorded, together with bounded atomic-`Int` addition (`LetPrim(Add)`) and atomic-Boolean-`Not` (`LetPrim(Not)`) extensions before `Jump(__answer)`; `!` admits only a Bool literal or already-bound Bool local, while nested/non-atomic/wider unary forms fail closed pending ANF/general lowering. General source/Core and production realization remain deferred. |
| [TASK-2004](tasks/TASK-2004-core-cps-production-boundary-decision.md) | Decide Core/CPS production-boundary status | In progress — TASK-2014 Path B keeps general routes closed under `CheckedCoreCpsClosedAdmission`, while `run`/`run_file` and the CLI runnable/trace helpers terminalize the handler-free pure subset and zero-input canonical bootstrap terminalizes the bounded nested-constructor subset via sealed checked CPS. Recursive `Return v` crosses as CPS `Value`; nested `Raise`/`Handle` and unsupported lowering reject, with no direct evaluator/provider/frame/async host path. Migrated source tests retain parse/check/import/binding evidence and wrapper profiles prove zero host evidence after closed admission. CLI bootstrap success/timeout controls use only the exact unit-`Ok` `Result<(), RuntimeError>` fixture, not nested-match lowering. Input bootstrap, the remaining CLI route matrix, application admission, registry/provider binding, authorized frame construction/lookup, async hosting, and canonical terminal envelopes remain open. |
| [TASK-2005](tasks/TASK-2005-direct-runtime-core-cps-semantic-parity.md) | Establish direct-runtime/Core-CPS semantic parity or bounded divergence | In progress — a fail-closed report has paired literal-return, hand-authored v3, literal-source, and exact lexical-source `int_add` values; two literal case-bound source Boolean-`Not` witnesses (`!true → Bool(false)`, `!false → Bool(true)`); and two separately closed lexical Boolean-`Not` witnesses: exact `let flag = true; return !flag → Bool(false)` and `let flag = false; return !flag → Bool(true)`, each with `LetVal flag → LetPrim(Not, Var(flag)) → Jump(__answer)`. Cross-case literal swaps/nesting and each lexical witness's binding, identity, and nesting tampering reject at corpus load; all other unary forms remain excluded. Each witness is differential-only private evidence and grants no production/direct-evaluator fallback. It also has paired V4 and source-derived literal-`If` true/false branch selection (`SEM-CPS-IF-001`); one `explicit_missing_discharge` `TestClock` `EffectOp` projection (`SEM-EFFECT-MISSDISCHARGE-001`); and one exact `time::sleep(0)` private reference/private checked-frame pair. The legacy direct oracle is differential-only, restricted to the canonical non-symlink built-in corpus root and exact allowlisted case/source tuples; untrusted roots are unsupported and no production route invokes it. The time-sleep direct reference does not dispatch a production provider. Checked targets remain private/prototype; other required observables remain unsupported, two rule IDs remain provisional, and no broad parity claim exists |
...
| [TASK-2008](tasks/TASK-2008-json-variant-observable-projection.md) | Reconcile JSON `_variant` with canonical observable projection | In progress — versioned `ash run --format json` projects return, declared and bounded entry-execution traps (including division by zero), unreadable-input/parse/type/entry-verification pre-entry failures, and one declaration-only missing-`main` dry-run failure through the existing `entry_verification` envelope with stdout/exclusive-`--output` ownership. It also projects both malformed pre-read and source-aware unknown-selection configuration failures as coarse `pre_entry_failure/configuration` `run configuration is invalid`, admission rejection, canonical `time::sleep` timeout, and cooperative one-shot cancellation without telemetry. The dry-run control adds no success/execution or legacy ordinary projection claim; execution traps have nonempty reason and stdout/exclusive-`--output` coverage. Verification, engine, and invalid-exit-code outcomes remain outside that route. Complete observable/differential coverage remains required. |
| [TASK-2009](tasks/TASK-2009-rust-stable-baseline-1-97-1.md) | Align Ash and Verus on Rust 1.96.0 | Complete |
...
| [TASK-2016](tasks/TASK-2016-local-nominal-newtype-checking.md) | Typecheck local non-generic nominal newtypes on the normal program path | Complete — normal checking registers local non-generic newtypes before bodies, validates the sole declared tuple constructor against its representation, and preserves nominal non-coercion and sibling-wrapper distinction. TASK-2001 additionally supplies a source-local current-module non-generic irrefutable `let Ctor(binding)` bridge whose binding uses the representation type and rejects wrong constructors/arity. Opaque bodyless representations, direct/mutual recursion, ordinary-type/newtype collisions, and primitive/prelude shadowing reject deterministically; runtime erasure/execution, imported/generic/re-exported newtypes, `match`/`if let`/proof patterns, handlers, and broader cross-module behavior remain deferred. |
...
| [TASK-2024](tasks/TASK-2024-handler-local-effect-row-propagation.md) | Propagate one nonproduction handler-local effect row through Core/CPS inspection | Complete — bounded `forward_sleep` preserves declaration-resolved `TestClock::sleep` as the handled body `Raise`, `TestClock::wake(ms)` as the clause-body `Raise`, and exactly `{TestClock::wake}` as the local Core/CPS `Handle.row`; `other(ms)` and `wake(0)` reject. This is structural private inspection only: no continuation residual/multi-shot/runtime/provider/admission or production Core/CPS authority is claimed. |
...
| [TASK-2013](tasks/TASK-2013-source-handler-and-handle-lowering.md) | Implement source handlers and `handle ... with` lowering | In progress — parser/AST preserves canonical clauses/origins; checked declarations retain marker/signature and concrete clause facts. The closed-empty identity Core inspection slice preserves `MultiShotPure` resume evidence through validated/type-checked Core/CPS; TASK-2014 terminalizes only that exact `echo_sleep` result through an opaque Engine-issued admission. The handler prewalk now accepts plain/record ambient `do` binds, legacy tuple-ADT patterns retain positional payload typing, and top-level `true`/`false` exhaust `Bool`; none is source-to-Core realization. TASK-786 retains reserved `handle` and rejects unregistered `Proc`. Nonempty/open rows, ordered frame installation/TASK-1993 dispatch, providers/residuals, async timeout/cancellation, and general handlers remain excluded. |
| [TASK-2014](tasks/TASK-2014-source-handler-runtime-boundary-decision.md) | Decide the source-handler runtime boundary | In progress — Path B makes checked Core/CPS the sole production owner of admitted source and keeps every source form without validated typed lowering closed. `run`/`run_file` admit their handler-free pure subset and zero-input bootstrap its bounded constructor subset as entry-anchor-bound sealed checked CPS tokens. A same-Engine/exact-anchor closed-empty identity artifact with one exact root `SourceHandler` instruction executes only through an opaque Engine-issued V1 wrapper and private answer terminalization, yielding `Int(0)` without a provider. The exact typed `fn main() -> Null { time::sleep(<non-negative Int literal>) }` slice now seals one registered provider binding and separately authorized frame instruction, then privately constructs and dispatches that one frame through the Engine async checked-CPS driver. Its post-admission control is one deadline with cancellation > deadline > completion; CLI projects return/timeout/cancellation without telemetry and rejects `--trace`. Rows/residual rows never install frames. The real one-instruction admission proves only exact frame authority and admission-time row/issuer/anchor/binding rejection; ordered multi-frame/TASK-1993 production evidence awaits widened real admission. Generic handlers, `resume`/`done`, residual/open rows, remaining routes, and missing-admission/malformed-Core/handler-trap terminal taxonomy remain open. |
| [TASK-439](tasks/TASK-439-differential-conformance-harness-rust-first.md) | Existing sole owner of the canonical differential corpus and harness | In progress — six file-backed Phase-202 adapters cover exact paired return, paired mismatch, allowed-set external, primitive-domain trap, source-return continuation, and one `explicit_missing_discharge` `EffectOp` projection; TASK-2005 adds literal and exact lexical source-add, two literal case-bound source Boolean-`Not` witnesses (`!true → false`, `!false → true`), and two separately closed lexical witnesses: `let flag = true; return !flag → Bool(false)` and `let flag = false; return !flag → Bool(true)`, each with `LetVal flag → LetPrim(Not, Var(flag)) → Jump(__answer)`, plus source true/false literal-`If` bridge adapters. Literal swaps/nesting and each lexical witness's binding, identity, and nesting tampering reject at corpus load. All remain differential-only private/prototype evidence; general lets/arithmetic/conditionals and production/direct-evaluator fallback remain excluded. Frozen v1 return/trap/affine-empty-row-Jump plus strict v2 `LetVal { Int; Return(Var(binder)) }`, v3 `int_add(Int, Int)` LetPrim, and v4 literal `If(Bool, Return(Int), Return(Int))` fixtures run only through the distinct checked-CPS prototype target with fail-closed validation. TASK-2020/2021/2022/2023 close five manifest-local canonical-Core V1 controls: each admits one exact fixed `core_text` only, then parse/validate/typecheck/checked-lower/private-projects; the last two are true/false literal `If` controls with exact answer jumps and selected returns. Alternate text rejects before parsing, and direct runtime stays unsupported. The lexical add bridge is exactly `x=2`, `y=5`, `LetVal → LetVal → LetPrim(Add) → Jump`; source `If` is exactly `If(Bool, Jump(__answer, 7), Jump(__answer, 9))`, with altered branches rejected at corpus load. The missing-discharge adapter preserves distinct source admission and CPS runtime failure classes and compares only identity; legacy workflow IR v1 is superseded as harness input; full canonical/runtime-observable coverage remains open |
