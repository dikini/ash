...
- **Binding metadata revalidation:** Imported declaration visibility is checked against the acquired checked target; same-identity visibility drift rejects atomically before interface publication with `BindingVisibilityMismatch`. A forged imported lookup namespace or declaration kind is rejected at the same boundary with `BindingShapeMismatch`, before any interface can publish.
- **Updated focused target:** The qualified namespace, implementation-call, qualified implementation-operation, where-bound, transitive-row, callable-whole-row, role-row, policy-row, binding-metadata, and forged-binding-shape RED/GREEN slices leave the focused TASK-2073 integration target at 93/93, plus one dedicated finalizer unit witness.
...
- **Activation and implementation evidence:** `crates/ash-typeck/tests/task_2073_checked_module_finalization.rs` passes 93/93, and `canonical_checked_module_finalizer::tests::forged_imported_binding_namespace_rejects_atomically` passes as a dedicated unit witness. Existing positive and negative evidence remains as previously recorded; the qualified namespace, qualified implementation-operation, where-bound, transitive-row, callable-whole-row, role/policy-row, binding-metadata, and forged-shape evidence above extends the public implementation closure slice. These are tests, not proof or downstream execution/client parity.
