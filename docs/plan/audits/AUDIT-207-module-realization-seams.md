...
| Graph identity and expansion | `crates/ash-parser/src/canonical_module_graph.rs::CanonicalModuleGraphResolver` publishes an all-or-nothing parser graph keyed by `ModuleKey`. TASK-2074's public `CanonicalExpandedModuleGraph` now owns that graph, performs a bounded AST-only public-macro syntax prepass with deterministic provider ordering/cycles and provenance, rejects unsupported item generation, publishes exactly one shallowly expanded record per key, retains uses/module declarations/order/per-key sidecars, and returns anchored failures atomically. | Implement the approved `use path::(pattern)` structured token/hole selector, deterministic transport of every eligible public pattern+fixity+associativity+precedence variant, and activation into the existing syntax table; reject aliases, globs, raw-spelling authority, invalid dependencies, and callable activation. Generalized mixfix use-site parsing/elaboration is outside TASK-2074. | TASK-2067 (Complete parsed graph), TASK-2074 (In progress, partial/tested bounded expansion) |
